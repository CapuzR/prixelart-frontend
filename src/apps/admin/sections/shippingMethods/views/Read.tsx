...
import {
    ...
    Tooltip, Stack, Chip, Toolbar, TextField, Select, MenuItem, // Added Toolbar, TextField, Select, MenuItem
...
const ReadShippingMethods: React.FC = () => {
    ...
    const loadMethods = useCallback(async (showLoading = true) => {
        ...
        setError(null);
        try {
            const fetchedMethods = await fetchShippingMethods() as ShippingMethod[];
            if (fetchedMethods.some(method => !method._id)) { /* ... error handling ... */ console.error("Missing _id."); throw new Error("Incomplete data."); }
            setShippingMethods(fetchedMethods.map(m => ({ ...m, createdOn: new Date(m.createdOn) }))); // Ensure createdOn is Date object
        } catch (err: any) { /* ... error handling ... */ const message = err.message || "Error al cargar."; setError(message); showSnackBar(message); console.error("Error fetching:", err); }
    ...
    }, [shippingMethods, searchQuery, filters, order, orderBy, page, rowsPerPage]);
    ...
    const handleFilterChange = (event: ChangeEvent<{ name?: string | undefined; value: unknown }>) => {
        ...
        if (name === 'status') { // Only handling status filter here
    ...
    const handleOpenDeleteDialog = (method: ShippingMethod) => { /* ... */ if (!method._id) { showSnackBar("Falta ID."); return; } setMethodToDelete(method); setDialogOpen(true); };
    const handleCloseDialog = () => { /* ... */ if (isDeleting) return; setDialogOpen(false); setMethodToDelete(null); };
    const handleConfirmDelete = async () => { /* ... */ if (!methodToDelete?._id) { showSnackBar("Error: ID faltante."); setIsDeleting(false); handleCloseDialog(); return; } setIsDeleting(true); try { await deleteShippingMethod(methodToDelete._id.toString()); showSnackBar(`Método "${methodToDelete.name}" eliminado.`); setShippingMethods(prev => prev.filter(m => m._id !== methodToDelete._id)); handleCloseDialog(); } catch (err: any) { console.error("Error deleting:", err); const message = err.message || "Error al eliminar."; showSnackBar(message); handleCloseDialog(); } finally { setIsDeleting(false); } };

    // --- Update & Create Handling --- (Keep existing logic)
    const handleUpdate = (methodId: string) => { /* ... */ if (!methodId) { showSnackBar("Falta ID."); return; } navigate(`/admin/shipping-method/update/${methodId}`); }; // Adjust route
    const handleCreate = () => { /* ... */ navigate('/admin/shipping-method/create'); }; // Adjust route

    // --- Table Head Configuration ---
    interface HeadCell { id: ShippingMethodSortableColumns; label: string; numeric: boolean; disablePadding: boolean; width?: string; }
    const headCells: readonly HeadCell[] = [
        { id: 'name', numeric: false, disablePadding: false, label: 'Nombre', width: '35%' },
        { id: 'price', numeric: true, disablePadding: false, label: 'Precio', width: '15%' }, // Mark as numeric for alignment
        { id: 'active', numeric: false, disablePadding: false, label: 'Estado', width: '15%' }, // Sortable via 'active' boolean
        { id: 'createdOn', numeric: false, disablePadding: false, label: 'Creado el', width: '20%' }, // New column
    ];
    ...
    return (
        <>
            ...
            {error && !isLoading && shippingMethods.length === 0 && (<Alert severity="error" sx={{ m: 2 }}>{error}<Button onClick={() => loadMethods()} size="small">Reintentar</Button></Alert>)}
            {!isLoading && totalFilteredCount === 0 && (shippingMethods.length > 0 || searchQuery || filters.status !== 'all') && (<Alert severity="info" sx={{ m: 2 }}>No hay métodos que coincidan con la búsqueda o filtros aplicados.</Alert>)}
            {!isLoading && shippingMethods.length === 0 && !error && !searchQuery && filters.status === 'all' && (<Alert severity="info" sx={{ m: 2 }}>No se encontraron métodos de envío.</Alert>)}
            ...
            {!isLoading && totalFilteredCount > 0 && (
                <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                    <TableContainer>
                        <Table stickyHeader sx={{ minWidth: 750 }} aria-label="shipping methods table">
                            <TableHead>
                            ...
                            <TableBody>
                                {processedMethods.map((method) => (
                                    <TableRow hover key={method._id?.toString()} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                        ...
                                        <TableCell align="right">${!isNaN(parseFloat(method.price)) ? parseFloat(method.price).toFixed(2) : 'N/A'}</TableCell>
                                        ...
                                        {/* Created On Date */}
                                        <TableCell>{method.createdOn instanceof Date ? method.createdOn.toLocaleDateString() : 'N/A'}</TableCell>
