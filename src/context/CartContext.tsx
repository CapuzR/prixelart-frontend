...
import { Selection } from "../types/product.types"
...
export const CartProvider: React.FC<MyComponentProps> = ({ children }) => {
  ...
  const calculateCartTotals = (lines: CartLine[]) => {
    ...
    return {
      ...
      totalUnits,
      cartDiscount,
      totalDiscount: subTotal - cartDiscount,
    }
  }
  ...
  const addOrUpdateItemInCart = async (
    ...
    try {
      ...
      const isSameSelection = (sel1?: Selection, sel2?: Selection): boolean => {
        if (!sel1 || !sel2) return false

        return (
          sel1.name === sel2.name &&
          sel1.value === sel2.value &&
          JSON.stringify(sel1.attributes) === JSON.stringify(sel2.attributes)
        )
      }
      ...
      if (lineId) {
        ...
        if (existingLineIndex !== -1) {
          ...
          const updatedSubtotal =
            (item.price - item?.discount - updatedDiscount) * updatedQuantity
          ...
          console.warn(
            "Line with specified lineId not found. Adding as new line."
          )
        }
      } else {
        const existingLineIndex = updatedLines.findIndex(
    ...
    try {
      const updatedLines = cart.lines.map((line) => {
        if (line.id === lineId) {
          ...
          const updatedSubtotal =
            updates.subtotal ??
            (line.item.price - line.item.discount - updatedDiscount) *
              updatedQuantity
  ...
  const deleteElementInItem = async (id: string, type: "producto" | "arte") => {
    try {
      const updatedLines = cart.lines
        ...
        .filter(
          (line): line is CartLine => !!line.item.product || !!line.item.art
        ) // Keep only lines with at least one valid element
