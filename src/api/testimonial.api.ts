...
export const createTestimonial = async (data: Omit<Testimonial, '_id'>): Promise<Testimonial> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error('Backend reported failure creating testimonial:', response.data.message);
            throw new Error(response.data.message || "Failed to create testimonial");
        }

        if (!response.data.result) {
            console.error('Backend reported success but no testimonial data was returned.');
...
export const readAllTestimonial = async (): Promise<Testimonial[]> => {
    ...
    try {
        ...
        const sortedTestimonials = testimonials.sort((a, b) => (a.position ?? 0) - (b.position ?? 0));
...
export const getTestimonialById = async (id: string): Promise<Testimonial> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure fetching testimonial '${id}':`, response.data.message);
            throw new Error(response.data.message || `Failed to fetch testimonial '${id}'`);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no data for testimonial '${id}'.`);
...
export const updateTestimonial = async (id: string, data: Partial<Testimonial>): Promise<Testimonial> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure updating testimonial '${id}':`, response.data.message);
            throw new Error(response.data.message || `Failed to update testimonial '${id}'`);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no testimonial data returned after update for '${id}'.`);
...
export const updateTestimonialOrder = async (orderedIds: string[]): Promise<boolean> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error('Backend reported failure updating testimonial order:', response.data.message);
            throw new Error(response.data.message || "Failed to update testimonial order");
        }
...
export const deleteTestimonial = async (id: string): Promise<Testimonial> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure deleting testimonial '${id}':`, response.data.message);
            throw new Error(response.data.message || `Failed to delete testimonial '${id}'`);
        }

        if (!response.data.result) {
            // Assuming backend returns the deleted object for consistency. Adjust .
            console.warn(`Backend reported success deleting testimonial '${id}' but returned no data.`);
