...
export const getRandomArt = async (): Promise<Art> => {
    try {
        const response = await axios.get<PrixResponse>(`${import.meta.env.VITE_BACKEND_URL}/art/random`);
...
export const getArtsByPrixer = async (
...
): Promise<PaginatedArtsResult | null> => { // Adjusted to return null on failure to align with some component logic
    ...
    try {
        ...
        throw new Error(e instanceof Error ? e.message : 'An unexpected error occurred while fetching arts.');
    }
};
...
export const updateArt = async (id: string, artData: Partial<Art>): Promise<PrixResponse> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure updating art`, response.data.message);
            throw new Error(response.data.message || `Failed to update art `);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no art data returned after update.`);
...
export const deleteArt = async (id: string): Promise<Art> => {
    const base_url =
        import.meta.env.VITE_BACKEND_URL + "/art/delete/" + id
    try {
