...
export const getOrderArchives = async (
  ...
  try {
    ...
    if (!response.data.success || !response.data.result) {
      const errorMsg =
        response.data.message || "Backend failed to fetch orders";
      console.error(
        "Backend reported failure fetching orders:",
        errorMsg,
        response.data,
      );
...
export const getOrderArchiveById = async (
  id: string,
): Promise<OrderArchive> => {
...
export const updateOrderArchive = async (
  id: string,
  orderArchiveData: Partial<OrderArchive>,
): Promise<OrderArchive> => {
  ...
  try {
    ...
    if (!response.data.success) {
      console.error(
        `Backend reported failure updating orderArchive:`,
        response.data.message,
      );
