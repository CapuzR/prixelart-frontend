  ...
  const finalPrice = (item) => {
    ...
    if (item.product.modifyPrice) {
      ...
      if (discount?.type === 'Porcentaje') {
        let op = Number((unitPrice - (unitPrice / 100) * discount.value) * item.quantity);
        unitPrice = op;
        return unitPrice;
    ...
    <React.Fragment>
      ...
      >
        ...
        {props.type === 'Depósito' ? (
          order?.requests.map(
            (item, index) =>
              item.art.prixerUsername === JSON.parse(localStorage.getItem('token')).username && (
                <>
                  <div
                    style={{
                      display: 'flex',
                  ...
                  >
                    <Typography variant="h6" style={{ marginTop: -38, textAlign: 'center' }}>
                      {`Orden #${order.orderId}`}
                    </Typography>
                    <Typography variant="body1" style={{ textAlign: 'center', marginBottom: 6 }}>
                    ...
                    <div style={{ padding: 10 }}>
                      ...
                      <div style={{ marginTop: 10 }}>{'Cantidad: ' + item.quantity}</div>
                      <div>
                        {'Precio unitario: $' +
                          Number(
                            item.product.finalPrice ||
                      ...
                      {typeof item.product.discount === 'string' && !item.product.modifyPrice && (
                        <div>{'Descuento: ' + getDis(item.product.discount)}</div>
                      )}
                      <div>
                        {'Precio final: $' +
          ...
          <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            <Typography variant="h6" style={{ marginTop: -38 }} color="secondary" align="center">
              {'Orden #'}
              {order?.orderId}
            </Typography>
              ...
              <>
                ...
                >
                  ...
                  <div style={{ padding: 10 }}>
                    ...
                    <div style={{ marginBottom: 10 }}>{'Prixer: ' + item.art?.prixerUsername}</div>
                    <div>{'Producto: ' + item.product.name}</div>
                    {item.product.selection &&
                      item.product.selection.attributes &&
                      item.product.attributes.map((a, i) => {
            ...
            >
              ...
              {(order?.shippingCost || order?.shippingData?.shippingMethod?.price) && (
                <div>
                  {'Envío: $' +
                    (
                      order?.shippingCost || order?.shippingData?.shippingMethod?.price
