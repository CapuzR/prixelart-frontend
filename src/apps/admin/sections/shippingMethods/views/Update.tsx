...
const UpdateShippingMethod: React.FC = () => {
  ...
  const fetchMethod = useCallback(async () => {
    if (!id) { /* ... error handling ... */ setErrorFetch("ID inválido."); setIsLoading(false); showSnackBar("ID inválido."); navigate("/admin/shipping-method/read"); return; }
    ...
    try {
      ...
      console.error("Failed fetch:", err); const message = err.message || "Error al cargar."; setErrorFetch(message); showSnackBar(message);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.price.trim()) { errors.price = "El precio es obligatorio."; }
    else { const priceValue = parseFloat(formData.price); if (isNaN(priceValue) || priceValue < 0) { errors.price = "Precio inválido (debe ser >= 0)."; } }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload: Partial<ShippingMethod> = { // Use Partial as we only send editable fields
      name: formData.name,
  ...
  const handleCancel = () => { navigate("/admin/shipping-method/read"); };
  ...
  return (
    <>
      ...
      <Paper elevation={3} sx={{ p: 3, mt: 2 }}>
        ...
        {errorFetch && !isLoading && (<Alert severity="error" sx={{ mb: 2 }}>{errorFetch}<Button onClick={fetchMethod} size="small">Reintentar</Button><Button onClick={handleCancel} size="small" color="secondary">Volver</Button></Alert>)}
        {!isLoading && !errorFetch && (
          <form onSubmit={handleSubmit} noValidate> {/* Use noValidate */}
            <Grid2 container spacing={3}>
              ...
              <Grid2 size={{ xs: 12, sm: 6 }}>
                <TextField
                  label="Precio" name="price" type="number" // Use type="number"
