...
export default function PayComission(props) {
  ...
  let consumersFiltered = props.consumers.filter(
  ...
  const modifyPrice = (index, newCom) => {
    setComissions((prevComissions) =>
      prevComissions.map((item, i) =>
        i === index ? { ...item, value: Number(newCom) } : item
      )
    )
  }
    ...
    if (
      ...
      await axios.post(url, data).then(async (res) => {
        if (res.data.success === false) {
          ...
          const url2 =
            process.env.REACT_APP_BACKEND_URL + "/order/updateComissions"
