import React, { useState, useEffect, ChangeEvent, FormEvent, SyntheticEvent, useMemo } from 'react';
...
interface OverrideFormState extends ApplicableEntity { tempId: string; }
interface RecipientFormState extends ApplicableEntity { tempId: string; }

interface SurchargeFormState extends Omit<Surcharge, '_id' | 'applicableProducts' | 'entityOverrides' | 'recipients' | 'dateRange'> {
...
interface ImageItemOption { // For Arts
    id: string;
    label: string;
    imageUrl?: string;
}
...
interface SurchargeValidationErrors {
    ...
    applicableProducts?: string; applicableArts?: string;
    ...
    recipients?: { [index: number]: { type?: string; id?: string; adjustmentMethod?: string; customValue?: string } };
    summary?: string;
}

// --- Helper type for single errors ---
type SingleRuleErrors = NonNullable<SurchargeValidationErrors['entityOverrides']>[number]; // Can reuse
...
const UpdateSurcharge: React.FC = () => {
    ...
    const [expandedAccordion, setExpandedAccordion] = useState<string | false>('applicability'); // Default expanded section
    ...
    const productVariantOptions = useMemo((): ProductOrVariantOption[] => {
        ...
        allProducts.forEach(product => {
            if (!product._id || !product.variants || product.variants.length === 0) return;
            const productIdStr = product._id.toString();
            const productImageUrl = getProductImageUrl(product);

            // Calculate Price Ranges for "Todo el producto"
            let minPublic = Infinity, maxPublic = -Infinity, minPrixer = Infinity, maxPrixer = -Infinity;
            let validPricesFound = false;
            product.variants.forEach(variant => {
                const publicP = parseFloat(variant.publicPrice); const prixerP = parseFloat(variant.prixerPrice);
                if (!isNaN(publicP)) { minPublic = Math.min(minPublic, publicP); maxPublic = Math.max(maxPublic, publicP); validPricesFound = true; }
                if (!isNaN(prixerP)) { minPrixer = Math.min(minPrixer, prixerP); maxPrixer = Math.max(maxPrixer, prixerP); validPricesFound = true; }
            });
            let priceRangeStr = '(Precios N/A)';
            if (validPricesFound) {
                const formatRange = (min: number, max: number, prefix: string): string => {
                    if (min === Infinity || max === -Infinity) return `${prefix}: ${formatPrice(null)}`; // Use formatPrice
            ...
            options.push({
                id: productIdStr, label: `${product.name} (Todo el producto) ${priceRangeStr}`,
                isProduct: true, productId: productIdStr, productName: product.name, imageUrl: productImageUrl
            });
            ...
            product.variants.forEach(variant => {
                ...
                options.push({
                    ...
                    variantId: variant._id, variantName: variant.name, imageUrl: variantImageUrl
                });
            });
        });
        return options;
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            if (!id) {
                setErrorFetch("ID de recargo inválido."); setIsLoadingData(false); setIsLoadingProducts(false); showSnackBar("ID de recargo inválido."); navigate("/admin/surcharges/read"); return;
            }
            setIsLoadingData(true); setIsLoadingProducts(true); setErrorFetch(null); setValidationErrors(null);
            ...
            try {
                ...
                const validProducts = productsData.filter(p => p._id && p.variants && p.variants.length > 0);
                ...
                setFormData(prev => ({ // Use previous state to avoid race conditions 
                    ...
                    appliesToAllProducts: surchargeData.appliesToAllProducts ?? (surchargeData.applicableProducts || []).length === 0, // Default true if empty
                    appliestoAllArts: surchargeData.appliestoAllArts,
                    applicableProducts: [],
                    applicableArts: [],
                    entityOverrides: (surchargeData.entityOverrides || []).map(ov => ({ ...ov, tempId: uuidv4() })),
                    recipients: (surchargeData.recipients || []).map(rec => ({ ...rec, tempId: uuidv4() })),
                }));

                // Set initial date picker state
                setStartDate(surchargeData.dateRange?.start ? dayjs(surchargeData.dateRange.start) : null);
                setEndDate(surchargeData.dateRange?.end ? dayjs(surchargeData.dateRange.end) : null);
    ...
    useEffect(() => {
        ...
        if (!isLoadingData && !isLoadingProducts && originalSurcharge && allProducts.length > 0 && productVariantOptions.length > 0) {

            const initialApplicableVariantIds = new Set<string>(); // Stores "productId_variantId"

            if (!originalSurcharge.appliesToAllProducts && originalSurcharge.applicableProducts) {
                originalSurcharge.applicableProducts.forEach(savedProdRef => {
                    ...
                    if (Array.isArray(savedProdRef) && savedProdRef.length === 2 && savedProdRef[0] && savedProdRef[1]) {
                        productId = savedProdRef[0];
                        variantId = savedProdRef[1];
                    } else if (Array.isArray(savedProdRef) && savedProdRef.length === 1 && savedProdRef[0]) {
                    ...
                    } else if (productId) { // Expand whole product
                        const product = allProducts.find(p => p._id!.toString() === productId);
                        if (product?.variants) {
                            product.variants.forEach(v => { if (v._id) initialApplicableVariantIds.add(`${productId}_${v._id}`); });
                        }
                    }
                });
            }
            ...
            const finalInitialApplicableProducts: [string, string][] = Array.from(initialApplicableVariantIds).map(idPair => {
            ...
            const preselectedOptions = productVariantOptions.filter(opt =>
                !opt.isProduct && initialApplicableVariantIds.has(opt.id)
            );
            ...
            if (finalInitialApplicableProducts.length > 0 || (originalSurcharge?.entityOverrides?.length ?? 0 > 0) || (originalSurcharge?.recipients?.length ?? 0 > 0) || originalSurcharge?.dateRange) {
    ...
    }, [isLoadingData, isLoadingProducts, originalSurcharge, allProducts, productVariantOptions]); // Dependencies
    ...
    const handleInputChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        if (validationErrors && validationErrors[name as keyof SurchargeValidationErrors]) { setValidationErrors(prevErrors => { if (!prevErrors) return null; const updated = { ...prevErrors }; delete updated[name as keyof SurchargeValidationErrors]; const keys = Object.keys(updated).filter(k => k !== 'summary'); return keys.length > 0 || updated.summary ? updated : null; }); }
    };
    const handleSelectChange = (event: ChangeEvent<{ name?: string; value: unknown }>) => {
        ...
        setFormData(prev => ({ ...prev, [name]: value as AdjustmentMethod | Entity }));
        if (validationErrors && validationErrors[name as keyof SurchargeValidationErrors]) { setValidationErrors(prevErrors => { if (!prevErrors) return null; const updated = { ...prevErrors }; delete updated[name as keyof SurchargeValidationErrors]; const keys = Object.keys(updated).filter(k => k !== 'summary'); return keys.length > 0 || updated.summary ? updated : null; }); }
    };
    const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (name === 'appliesToAllProducts' && validationErrors?.applicableProducts) {
            setValidationErrors(prev => prev ? ({ ...prev, applicableProducts: undefined }) : null);
        }
    };
    const handleProductVariantChange = (event: SyntheticEvent, newValue: ProductOrVariantOption[]) => {
        ...
        newValue.forEach(option => {
            if (option.isProduct) {
                ...
                if (product?.variants) {
                    product.variants.forEach(variant => { if (variant._id) finalApplicableVariants.add(`${product._id}_${variant._id}`); });
                }
            } else if (option.variantId) { // Ensure it's a variant option
        ...
        const newApplicableProducts: [string, string][] = Array.from(finalApplicableVariants).map(idPair => {
            const [prodId, varId] = idPair.split('_'); return [prodId, varId];
        });
    ...
    const handleStartDateChange = (newValue: Dayjs | null) => {
        ...
        if (validationErrors?.startDate || validationErrors?.endDate || validationErrors?.dateRange) { setValidationErrors(prevErrors => { if (!prevErrors) return null; const updated = { ...prevErrors }; delete updated.startDate; delete updated.endDate; delete updated.dateRange; const keys = Object.keys(updated).filter(k => k !== 'summary'); return keys.length > 0 || updated.summary ? updated : null; }); }
    };
    ...
    const handleAddOverride = () => { setFormData(prev => ({ ...prev, entityOverrides: [...prev.entityOverrides, { tempId: uuidv4(), type: 'user', adjustmentMethod: 'percentage', customValue: 0, id: '', name: '' }] })); if (validationErrors?.entityOverrides) { setValidationErrors(prevErrors => { if (!prevErrors) return null; const updated = { ...prevErrors }; delete updated.entityOverrides; const keys = Object.keys(updated).filter(k => k !== 'summary'); return keys.length > 0 || updated.summary ? updated : null; }); } };
    const handleOverrideChange = (tempId: string, field: keyof ApplicableEntity, value: string | number | Entity | AdjustmentMethod) => { setFormData(prev => ({ ...prev, entityOverrides: prev.entityOverrides.map((ov: OverrideFormState) => ov.tempId === tempId ? { ...ov, [field]: value } : ov) })); if (validationErrors?.entityOverrides) { const index = formData.entityOverrides.findIndex(ov => ov.tempId === tempId); if (index === -1) return; const errorFieldKey = field as keyof SingleRuleErrors; if (validationErrors.entityOverrides[index]?.[errorFieldKey]) { setValidationErrors(prevErrors => { /*... clear specific error logic ...*/  if (!prevErrors?.entityOverrides?.[index]) return prevErrors; const updatedOverrides = { ...(prevErrors.entityOverrides) }; const specificOverrideErrors = { ...updatedOverrides[index] }; delete specificOverrideErrors[errorFieldKey]; if (Object.keys(specificOverrideErrors).length === 0) { delete updatedOverrides[index]; } else { updatedOverrides[index] = specificOverrideErrors; } const finalOverrides = Object.keys(updatedOverrides).length > 0 ? updatedOverrides : undefined; const newErrors = { ...prevErrors, entityOverrides: finalOverrides }; if (!newErrors.entityOverrides) delete newErrors.entityOverrides; const keys = Object.keys(newErrors).filter(k => k !== 'summary'); return keys.length > 0 || newErrors.summary ? newErrors : null; }); } } };
    const handleRemoveOverride = (tempIdToRemove: string) => { const indexToRemove = formData.entityOverrides.findIndex(ov => ov.tempId === tempIdToRemove); setFormData(prev => ({ ...prev, entityOverrides: prev.entityOverrides.filter(ov => ov.tempId !== tempIdToRemove) })); if (validationErrors?.entityOverrides && indexToRemove !== -1) { setValidationErrors(prevErrors => { /*... clear/shift override errors ...*/ if (!prevErrors?.entityOverrides) return prevErrors; const currentOverrideErrors = { ...prevErrors.entityOverrides }; const updatedOverrideErrors: SurchargeValidationErrors['entityOverrides'] = {}; let errorFound = false; Object.keys(currentOverrideErrors).forEach(keyIndexStr => { const keyIndex = parseInt(keyIndexStr, 10); if (keyIndex === indexToRemove) return; const newIndex = keyIndex > indexToRemove ? keyIndex - 1 : keyIndex; updatedOverrideErrors[newIndex] = currentOverrideErrors[keyIndex]; errorFound = true; }); const finalOverrides = errorFound ? updatedOverrideErrors : undefined; const newErrors = { ...prevErrors, entityOverrides: finalOverrides }; if (!newErrors.entityOverrides) delete newErrors.entityOverrides; const keys = Object.keys(newErrors).filter(k => k !== 'summary'); return keys.length > 0 || newErrors.summary ? newErrors : null; }); } };
    const handleAddRecipient = () => { setFormData(prev => ({ ...prev, recipients: [...prev.recipients, { tempId: uuidv4(), type: 'user', adjustmentMethod: 'percentage', customValue: 0, id: '', name: '' }] })); if (validationErrors?.recipients) { setValidationErrors(prevErrors => { if (!prevErrors) return null; const updated = { ...prevErrors }; delete updated.recipients; const keys = Object.keys(updated).filter(k => k !== 'summary'); return keys.length > 0 || updated.summary ? updated : null; }); } };
    const handleRecipientChange = (tempId: string, field: keyof ApplicableEntity, value: string | number | Entity | AdjustmentMethod) => { setFormData(prev => ({ ...prev, recipients: prev.recipients.map((rec: RecipientFormState) => rec.tempId === tempId ? { ...rec, [field]: value } : rec) })); if (validationErrors?.recipients) { const index = formData.recipients.findIndex(rec => rec.tempId === tempId); if (index === -1) return; const errorFieldKey = field as keyof SingleRuleErrors; if (validationErrors.recipients[index]?.[errorFieldKey]) { setValidationErrors(prevErrors => { /*... clear specific error logic ...*/ if (!prevErrors?.recipients?.[index]) return prevErrors; const updatedRecipients = { ...(prevErrors.recipients) }; const specificRecipientErrors = { ...updatedRecipients[index] }; delete specificRecipientErrors[errorFieldKey]; if (Object.keys(specificRecipientErrors).length === 0) { delete updatedRecipients[index]; } else { updatedRecipients[index] = specificRecipientErrors; } const finalRecipients = Object.keys(updatedRecipients).length > 0 ? updatedRecipients : undefined; const newErrors = { ...prevErrors, recipients: finalRecipients }; if (!newErrors.recipients) delete newErrors.recipients; const keys = Object.keys(newErrors).filter(k => k !== 'summary'); return keys.length > 0 || newErrors.summary ? newErrors : null; }); } } };
    const handleRemoveRecipient = (tempIdToRemove: string) => { const indexToRemove = formData.recipients.findIndex(rec => rec.tempId === tempIdToRemove); setFormData(prev => ({ ...prev, recipients: prev.recipients.filter(rec => rec.tempId !== tempIdToRemove) })); if (validationErrors?.recipients && indexToRemove !== -1) { setValidationErrors(prevErrors => { /*... clear/shift recipient errors ...*/ if (!prevErrors?.recipients) return prevErrors; const currentRecipientErrors = { ...prevErrors.recipients }; const updatedRecipientErrors: SurchargeValidationErrors['recipients'] = {}; let errorFound = false; Object.keys(currentRecipientErrors).forEach(keyIndexStr => { const keyIndex = parseInt(keyIndexStr, 10); if (keyIndex === indexToRemove) return; const newIndex = keyIndex > indexToRemove ? keyIndex - 1 : keyIndex; updatedRecipientErrors[newIndex] = currentRecipientErrors[keyIndex]; errorFound = true; }); const finalRecipients = errorFound ? updatedRecipientErrors : undefined; const newErrors = { ...prevErrors, recipients: finalRecipients }; if (!newErrors.recipients) delete newErrors.recipients; const keys = Object.keys(newErrors).filter(k => k !== 'summary'); return keys.length > 0 || newErrors.summary ? newErrors : null; }); } };
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.appliesToAllProducts && selectedProductVariants.length === 0) { errors.applicableProducts = "Seleccione productos/variantes o marque 'Aplica a todos'."; errorMessages.push("Productos/Variantes"); }
        ...
        const overrideErrors: SurchargeValidationErrors['entityOverrides'] = {}; let hasOverrideError = false;
        formData.entityOverrides.forEach((ov, index) => { /* ... override validation logic ... */
            const currentErrors: SingleRuleErrors = {}; let overrideErrorFields: string[] = [];
            ...
            if (Object.keys(currentErrors).length > 0) { overrideErrors[index] = currentErrors; if (!hasOverrideError) { errorMessages.push(`Excepción #${index + 1} (${overrideErrorFields.join(', ')})`); } hasOverrideError = true; }
        });
        if (hasOverrideError) { errors.entityOverrides = overrideErrors; }

        // Recipients Validation
        const recipientErrors: SurchargeValidationErrors['recipients'] = {}; let hasRecipientError = false;
        formData.recipients.forEach((rec, index) => { /* ... recipient validation logic ... */
            const currentErrors: SingleRuleErrors = {}; let recipientErrorFields: string[] = [];
            if (!rec.type) { currentErrors.type = "Requerido"; recipientErrorFields.push("Tipo Entidad"); }
            if (!rec.id?.trim()) { currentErrors.id = "ID Requerido"; recipientErrorFields.push("ID Entidad"); }
            if (!rec.adjustmentMethod) { currentErrors.adjustmentMethod = "Requerido"; recipientErrorFields.push("Método Dest."); }
            const customValueNum = Number(rec.customValue);
            if (rec.customValue === undefined || rec.customValue === null || isNaN(customValueNum)) { currentErrors.customValue = "Valor numérico requerido."; recipientErrorFields.push("Valor Destinatario"); }
            else if (rec.adjustmentMethod === 'percentage' && (customValueNum < 0 || customValueNum > 1)) { currentErrors.customValue = "Valor entre 0 y 1."; recipientErrorFields.push("Valor Destinatario (%)"); }
            else if (rec.adjustmentMethod === 'absolute' && customValueNum < 0) { currentErrors.customValue = "Valor no negativo."; recipientErrorFields.push("Valor Destinatario ($)"); }
            if (Object.keys(currentErrors).length > 0) { recipientErrors[index] = currentErrors; if (!hasRecipientError) { errorMessages.push(`Destinatario #${index + 1} (${recipientErrorFields.join(', ')})`); } hasRecipientError = true; }
        });
        if (hasRecipientError) { errors.recipients = recipientErrors; }
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        const finalOverrides: ApplicableEntity[] = formData.entityOverrides.map(ov => { const { tempId, ...rest } = ov; const data: ApplicableEntity = { ...rest, id: rest.id?.trim() || undefined, customValue: (rest.customValue !== undefined && !isNaN(Number(rest.customValue))) ? Number(rest.customValue) : undefined }; Object.keys(data).forEach(k => data[k as keyof ApplicableEntity] === undefined && delete data[k as keyof ApplicableEntity]); return data; });
        // Clean recipients (same as CreateSurcharge)
        const finalRecipients: ApplicableEntity[] = formData.recipients.map(rec => { const { tempId, ...rest } = rec; const data: ApplicableEntity = { ...rest, id: rest.id, customValue: (rest.customValue !== undefined && !isNaN(Number(rest.customValue))) ? Number(rest.customValue) : undefined }; Object.keys(data).forEach(k => data[k as keyof ApplicableEntity] === undefined && delete data[k as keyof ApplicableEntity]); return data; });
        ...
        const finalApplicableProducts = formData.appliesToAllProducts
            ? []
            : selectedProductVariants.map(opt => (opt.isProduct ? [opt.productId] : [opt.productId, opt.variantId]) as [string, string?]);
        ...
        try {
            ...
            if (response) {
                ...
                throw new Error("La actualización del recargo falló (respuesta vacía).");
            }
        } catch (err: any) {
            console.error("Failed to update surcharge:", err);
            const message = err?.response?.data?.message || err.message || "Error al actualizar el recargo.";
    ...
    return (
        <>
            {/* --- Update Title --- */}
            <Title title={`Actualizar Recargo: ${originalSurcharge?.name || (id ? 'Cargando...' : 'Inválido')}`} />
            {/* --- End Title Update --- */}
                ...
                {errorFetch && !isLoading && (
                    <Alert severity="error" sx={{ mb: 2 }} action={
                        <Stack direction="row" spacing={1}>
                            <Button onClick={handleCancel} size="small" color="inherit">Volver</Button>
                        </Stack>
                    }>{errorFetch}</Alert>
                )}
                ...
                {!isLoading && !errorFetch && (
                    <form onSubmit={handleSubmit} noValidate>
                        ...
                        {validationErrors?.summary && (
                            <Alert severity="error" icon={<WarningAmberIcon fontSize="inherit" />} sx={{ mb: 3 }}>
                                {validationErrors.summary}
                            </Alert>
                        )}
                        {/* --- END Error Summary --- */}
                        ...
                        <Grid2 container spacing={3}>
                            ...
                            <Grid2 size={{ xs: 12, sm: 6 }}><TextField label="Nombre del Recargo" name="name" value={formData.name} onChange={handleInputChange} required fullWidth disabled={isSubmitting} error={!!validationErrors?.name} helperText={validationErrors?.name} /></Grid2>
                            ...
                            <Grid2 size={{ xs: 12, sm: 6 }}> <FormControl fullWidth required error={!!validationErrors?.adjustmentMethod}><InputLabel id="adj-m-lbl">Método Base</InputLabel><Select labelId="adj-m-lbl" name="adjustmentMethod" value={formData.adjustmentMethod} label="Método Base" onChange={handleSelectChange as any} disabled={isSubmitting}>{ADJUSTMENT_METHOD_OPTIONS.map(opt => (<MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>))}</Select><FormHelperText>{validationErrors?.adjustmentMethod}</FormHelperText></FormControl> </Grid2>
                            <Grid2 size={{ xs: 12, sm: 6 }}> <TextField label="Valor Base" name="defaultValue" type="number" value={formData.defaultValue} onChange={handleInputChange} required fullWidth disabled={isSubmitting} helperText={validationErrors?.defaultValue || (formData.adjustmentMethod === 'percentage' ? "Ej: 0.15 para 15%" : "Ej: 5.00")} error={!!validationErrors?.defaultValue} InputProps={{ endAdornment: formData.adjustmentMethod === 'percentage' ? <InputAdornment position="end">% (Decimal)</InputAdornment> : undefined, startAdornment: formData.adjustmentMethod === 'absolute' ? <InputAdornment position="start">$</InputAdornment> : undefined, inputProps: formData.adjustmentMethod === 'percentage' ? { step: "0.01", min: "0", max: "1" } : { step: "0.01", min: "0" } }} /> </Grid2>

                            {/* --- Accordion Sections --- */}
                            <Grid2 size={{ xs: 12 }}>
                                {/* Applicability Accordion */}
                                <Accordion expanded={expandedAccordion === 'applicability'} onChange={handleAccordionChange('applicability')} sx={{ '&.Mui-expanded': { margin: '16px 0' } }}>
                                    <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="applicability-content" id="applicability-header"> <Typography variant="h6" component="div">Aplicabilidad Base</Typography> </AccordionSummary>
                                    <AccordionDetails>
                                                ...
                                                <Box sx={{ border: 1, borderColor: 'divider', borderRadius: 1, p: 2 }}>
                                                    ...
                                                    {!formData.appliesToAllProducts && (
                                                        <Autocomplete
                                                            ...
                                                            loading={isLoadingProducts && productVariantOptions.length === 0}
                                                            ...
                                                            renderInput={(params) => (
                                                                <TextField
                                                                    ...
                                                                    helperText={validationErrors?.applicableProducts && validationErrors.applicableProducts !== " " ? validationErrors.applicableProducts : null}
                                                                />
                                                            )}
                                                        />
                                                    )}
                                ...
                                <Accordion expanded={expandedAccordion === 'dateRange'} onChange={handleAccordionChange('dateRange')} sx={{ '&.Mui-expanded': { margin: '16px 0' } }}>
                                    <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="dateRange-content" id="dateRange-header"> <Typography variant="h6" component="div">Rango de Fechas (Opcional)</Typography> </AccordionSummary>
                                    <AccordionDetails>
                                        <Grid2 container spacing={2}>
                                            <Grid2 size={{ xs: 12, sm: 6 }}> <DatePicker label="Fecha de Inicio" value={startDate} onChange={handleStartDateChange} disabled={isSubmitting} slotProps={{ textField: { fullWidth: true, error: !!validationErrors?.startDate || !!validationErrors?.dateRange, helperText: validationErrors?.startDate || validationErrors?.dateRange } }} /> </Grid2>
                                            <Grid2 size={{ xs: 12, sm: 6 }}> <DatePicker label="Fecha de Fin" value={endDate} onChange={handleEndDateChange} disabled={isSubmitting} minDate={startDate || undefined} slotProps={{ textField: { fullWidth: true, error: !!validationErrors?.endDate || !!validationErrors?.dateRange, helperText: validationErrors?.endDate || validationErrors?.dateRange } }} /> </Grid2>
                                        </Grid2>
                                    </AccordionDetails>
                                </Accordion>
                                ...
                                <Accordion expanded={expandedAccordion === 'overrides'} onChange={handleAccordionChange('overrides')} sx={{ '&.Mui-expanded': { margin: '16px 0' } }}>
                                    <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="overrides-content" id="overrides-header"> <Typography variant="h6" component="div">Excepciones Específicas (Opcional)</Typography> </AccordionSummary>
                                    <AccordionDetails>
                                        {formData.entityOverrides.length === 0 && (<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>No se han añadido excepciones.</Typography>)}
                                        {formData.entityOverrides.map((override, index) => {
                                            const overrideErrors = validationErrors?.entityOverrides?.[index] || {}; return (
                                        ...
                                        <Button startIcon={<AddCircleOutlineIcon />} onClick={handleAddOverride} disabled={isSubmitting} variant="outlined" size="small" sx={{ mt: formData.entityOverrides.length > 0 ? 0 : 2 }}>Añadir Excepción</Button>
                                    </AccordionDetails>
                                </Accordion>

                                {/* Recipients Accordion */}
                                <Accordion expanded={expandedAccordion === 'recipients'} onChange={handleAccordionChange('recipients')} sx={{ '&.Mui-expanded': { margin: '16px 0' } }}>
                                    <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="recipients-content" id="recipients-header"> <Typography variant="h6" component="div">Destinatarios del Recargo (Opcional)</Typography> </AccordionSummary>
                                    <AccordionDetails>
                                        {formData.recipients.length === 0 && (<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>No se han añadido destinatarios. Si no se añaden, el recargo no se distribuirá.</Typography>)}
                                        {formData.recipients.map((recipient, index) => {
                                            const recipientErrors = validationErrors?.recipients?.[index] || {}; return (
                                        ...
                                        <Button startIcon={<AddCircleOutlineIcon />} onClick={handleAddRecipient} disabled={isSubmitting} variant="outlined" size="small" sx={{ mt: formData.recipients.length > 0 ? 0 : 2 }}>Añadir Destinatario</Button>
                                    </AccordionDetails>
                                </Accordion>
                            </Grid2>
                            ...
                            <Grid2 size={{ xs: 12 }}>
                                <Stack direction="row" justifyContent="flex-end" spacing={2} sx={{ mt: 3 }}>
                                    ...
                                    {/* --- Update Button Text --- */}
                                    <Button type="submit" variant="contained" color="primary" disabled={isSubmitting || isLoading /* Disable if loading data */} startIcon={isSubmitting ? <CircularProgress size={20} color="inherit" /> : null}>
