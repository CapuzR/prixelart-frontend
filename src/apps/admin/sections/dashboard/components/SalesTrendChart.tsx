...
import { Paper, Typography, Box, CircularProgress, useTheme, ToggleButtonGroup, ToggleButton } from '@mui/material';
...
const SalesTrendChart: React.FC<SalesTrendChartProps> = ({ orders, loading, startDate, endDate }) => {
    ...
    return (
        <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, height: '100%' }}>
            ...
            {loading ? (
                <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexGrow: 1, height: 300 }}>
                    <CircularProgress />
                </Box>
            ) : (
                <Box sx={{ height: 300 }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                            ...
                            <Tooltip formatter={(value: number) => metric === 'sales' ? `$${value.toFixed(2)}` : value.toLocaleString()} />
                            <Legend />
