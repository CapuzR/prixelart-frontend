...
export default function OrderDetails(props) {
  ...
  const PriceSelect = (item) => {
    if (typeof item.selectedPrixer?.username === 'string') {
      ...
      return getPVP(item, false, props?.dollarValue, props.discountList).toLocaleString('de-DE', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      });
    }
  ...
  const allowMockup = (item, index) => {
    if (item.product?.mockUp !== undefined && item.art.title !== 'Personalizado') {
      return (
        <div>
          <div
            style={{
  ...
  useEffect(() => {
    let prix = [];
    if (props.modalContent?.comissions && props.modalContent?.comissions.length > 0) {
          ...
          {props.permissions?.detailOrder ? (
            <>
              ...
              >
                  ...
                  <div style={{ display: 'flex' }}>
                        ...
                        >
                          ...
                          <div style={{ padding: 10 }}>
                            ...
                            {item.product?.autoCertified && (
                              ...
                              >
                                {'Certificado: ' +
                                  props.modalContent.requests[0].art.certificate.code +
                                  formatNumber(
                                    props.modalContent.requests[0].art.certificate.serial,
                                    2
                                  ) +
                                  formatNumber(
                                    props.modalContent.requests[0].art.certificate.sequence,
                                    3
                                  )}
                                <div />
                              </div>
                            ...
                            >
                              ...
                              <Select
                                input={<OutlinedInput />}
                                id="status"
                                value={item.product?.status ? item.product?.status : 'Por producir'}
                                onChange={(e) => {
                                  updateItemStatus(
                                    e.target.value,
                                    index,
                    ...
                    >
                      ...
                      >
                        ...
                        <div>{'Teléfono: ' + props.modalContent?.basicData.phone}</div>
                        <div>{'Email: ' + props.modalContent?.basicData.email}</div>
                        <div>{'Dirección: ' + props.modalContent?.basicData.address}</div>
                      </Grid>
                      ...
                      {props.modalContent.shippingData !== undefined && (
                        ...
                        >
                          ...
                          {props.modalContent.shippingData?.phone && (
                            <div>{'Teléfono: ' + props.modalContent?.shippingData?.phone}</div>
                          )}
                          {props.modalContent.shippingData?.shippingMethod && (
                            <div>
                              {'Método de entrega: ' +
                                props.modalContent?.shippingData?.shippingMethod.name}
                            </div>
                          )}
                          {props.modalContent.shippingData?.address ? (
                            <div>
                              {'Dirección de envío: ' + props.modalContent?.shippingData?.address}
                            </div>
                          ) : (
                            props.modalContent?.basicData?.address && (
                              <div>
                                {'Dirección de envío: ' + props.modalContent?.basicData?.address}
                              </div>
                            )
                          )}
                          {props.modalContent.shippingData?.shippingDate && (
                            <div>
                              {'Fecha de entrega: ' +
                                moment(props.modalContent?.shippingData?.shippingDate)?.format(
                                  'DD/MM/YYYY'
                                )}
                            </div>
                          )}
                        ...
                        >
                          ...
                          <div>
                            {props.modalContent.createdBy.username !== undefined &&
                              'Pedido creado por: ' + props.modalContent.createdBy.username}
                          </div>
                          ...
                          {props.modalContent.billingData.ci && (
                            <div>{'CI o RIF: ' + props.modalContent?.billingData.ci}</div>
                          )}
                          {props.modalContent.billingData.company && (
                            <div>{'Razón social: ' + props.modalContent?.billingData.company}</div>
                          )}
                          {props.modalContent.billingData.phone && (
                            <div>{'Teléfono: ' + props.modalContent?.billingData.phone}</div>
                          )}
                          {props.modalContent.billingData.address && (
                            <div style={{ marginBottom: 20 }}>
                              {'Dirección de cobro: ' + props.modalContent?.billingData.address}
                            </div>
                          )}
                        </Grid>
                      )}
                      ...
                      >
                        ...
                        <div>
                          {'Subtotal: $' +
                            Number(props.modalContent?.subtotal).toLocaleString('de-DE', {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                        </div>
                        <div>
                          IVA: $
                          {props.modalContent?.billingData?.orderPaymentMethod === 'Balance Prixer'
                            ? '0,00'
                            : Number(props.modalContent?.tax).toLocaleString('de-DE', {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                        </div>
                        <div>
                          {props.modalContent.shippingData?.shippingMethod &&
                            'Envío: $' +
                              Number(
                                props.modalContent?.shippingData?.shippingMethod?.price
                              ).toLocaleString('de-DE', {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                        ...
                        <div>
                          {'Total: $' +
                            Number(props.modalContent?.total).toLocaleString('de-DE', {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                        ...
                        {props.modalContent?.dollarValue && (
                          <div style={{ marginBottom: 10 }}>
                            {'Tasa del dólar: Bs' +
                              Number(props.modalContent?.dollarValue).toLocaleString('de-DE', {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                        ...
                        <div>
                          {'Forma de pago: ' + props.modalContent?.billingData?.orderPaymentMethod}
                        </div>
                ...
                ) : activeStep === 1 && props.modalContent?.paymentVoucher !== undefined ? (
                  <Paper
                    elevation={3}
                    style={{
                      width: 'fit-content',
                  ...
                  props.modalContent?.comissions.map((com, i) => (
                    ...
                    >
                      ...
                      <Typography
                        ...
                        variant="h6"
                        style={{
                          color: com.type === 'Depósito' && com.value > 0 ? 'green' : 'red',
                        }}
                      >
                        {com.type === 'Depósito' && '+'}
                        {'$' + com.value}
                ...
                >
                  ...
                  >
                    {item.product?.mockUp !== undefined && (
                      <div style={{ width: 210 }}>{allowMockup(item, index)}</div>
                    )}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'row' }}>
                        {item.product?.mockUp === undefined &&
                          item.product?.sources?.images.length > 0 && (
                            ...
                            >
                              <Img
                                src={item.product.thumbUrl || item.product?.sources?.images[0].url}
                                style={{
                                  maxWidth: 150,
                                  maxHeight: 150,
                                  borderRadius: 10,
                        ...
                        >
                          ...
                          >
                            ...
                            <Select
                              ...
                              value={item.product?.status ? item.product?.status : 'Por producir'}
                              onChange={(e) => {
                                updateItemStatus(e.target.value, index, props.modalContent.orderId);
                              }}
                            >
                              <MenuItem key={0} value={'Por producir'}>
                                Por producir
