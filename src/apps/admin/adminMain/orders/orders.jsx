...
export default function Orders(props) {
  ...
  const downloadOrders = async () => {
    ...
    orders.map((order, i) => {
      const v2 = {
        ...
        basicData:
          (order.basicData?.firstname || order.basicData?.name) + ' ' + order.basicData?.lastname,
      ...
      if (order.shippingData?.shippingMethod !== undefined) {
        shippingData = shippingData.concat(order.shippingData?.shippingMethod?.name);
      }
      let shippingDate;
      if (order.shippingData?.shippingDate !== undefined) {
        shippingDate = new Date(order.shippingData?.shippingDate).toLocaleDateString();
      }
      let prixer = '';
      let art = '';
      let product = '';
      ...
      order.requests.map((item) => {
        ...
        if (item.product.finalPrice !== undefined && typeof item.product.finalPrice === 'string') {
          ...
          price = ('$', Number(item.product.publicEquation.replace(/[,]/gi, '.')));
        } else if (
          item.product.prixerEquation !== undefined &&
          item.product.prixerEquation !== ''
        ) {
          price = ('$', item.product.prixerEquation);
        } else price = ('$', Number(item.product?.publicPrice?.from.replace(/[,]/gi, '.')));
  ...
  const payComission = async (order) => {
    for (let item of order.requests) {
      ...
      let consumersFiltered = consumers.filter((con) => con.consumerType === 'Prixer');
      ...
      if (ORGS !== undefined) {
        ...
        let p = ORGS?.agreement?.appliedProducts?.find((p) => p._id === item.product._id);
        let co = p?.variants?.length > 0 ? Number(p?.variants[0]?.cporg) : Number(p?.cporg);
        if (
          order.consumerData &&
        ...
        if (item.product.modifyPrice) {
          ...
          item.product.prixerEquation
            ? (unitPrice = Number(item.product?.prixerEquation?.replace(/[,]/gi, '.')))
            : (unitPrice = Number(item.product?.prixerPrice?.from?.replace(/[,]/gi, '.')));
        } else {
          item.product.publicEquation
            ? (unitPrice = Number(item.product?.publicEquation.replace(/[,]/gi, '.')))
            : (unitPrice = Number(item.product.publicPrice.from?.replace(/[,]/gi, '.')));
        }
        ...
        if (
          ...
          unitPrice = (unitPrice - unitPrice / 10) / (1 - item.art.comission / 100);
        }
  ...
  const handleChangePayStatus = async (order, payStatus) => {
    ...
    if (
      (payStatus === 'Pagado' || payStatus === 'Giftcard' || payStatus === 'Obsequio') &&
  ...
  return (
    <>
      ...
      <Grid container spacing={2} style={{ marginLeft: isDesktop ? '12px' : '' }}>
          ...
          <Paper className={fixedHeightPaper}>
            ...
            >
              ...
              <div>
                ...
                <Tooltip title="Eliminar filtros" style={{ height: 40, width: 40 }}>
                  <Fab
                    color="primary"
                    size="small"
                ...
                <Tooltip title="Recargar listado" style={{ height: 40, width: 40 }}>
                  <Fab
                    color="primary"
                    size="small"
                    onClick={handleRemoveFilters}
