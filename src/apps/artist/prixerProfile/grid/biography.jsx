...
export default function Biography(props) {
  ...
  const updateBio = async () => {
    ...
    formData.append('prixerId', JSON.parse(localStorage.getItem('token')).prixerId);
  ...
  const settings = {
    slidesToShow:
      images.length < 3 ? images.length : (isMobile && 2) || (isTab && 3) || (isDesktop && 3),
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 4000,
    speed: 500,
    ...
    <>
      ...
      <Grid container style={{ justifyContent: 'center', marginBottom: 20 }}>
        {openEdit ? (
          ...
          JSON.parse(localStorage.getItem('token'))?.username === props.prixerUsername ? (
          <Paper
            elevation={3}
            className={classes.paper}
