...
import { useSnackBar } from "@prixpon/context/GlobalContext"; // For notifications
...
const ALL_ORDER_STATUSES_MODAL: OrderStatus[] = [
  "Anulado",
  "Concretado",
  "En producción",
  "Por entregar",
...
const ALL_PAY_STATUSES_MODAL: OrderPayStatus[] = [
  "Pagado",
  "Anulado",
  "Obsequio",
  "Abonado",
...
const FINAL_ORDER_STATUSES_MODAL: OrderStatus[] = [
  "Concretado",
  "Entregado",
  "Anulado",
];
const UPDATABLE_TARGET_ORDER_STATUSES_MODAL: OrderStatus[] =
  ALL_ORDER_STATUSES_MODAL.filter(
    (s) => !FINAL_ORDER_STATUSES_MODAL.includes(s) || s === "Concretado",
  );
...
const OrderItem: React.FC<OrderItemProps> = ({ request }) => {
  ...
  const renderAttributes = (selection: OrderProduct["selection"]) => {
    ...
    if (typeof selection === "string") {
      ...
      const validStrings = selection.filter(
        (s): s is string => typeof s === "string",
      );
      if (validStrings.length > 0) {
        attributesToDisplay.push({
          name: "Valores",
          value: validStrings.join(", "),
        });
      }
      ...
      if (selectionObject.name) {
        attributesToDisplay.push({
          name: "Selección",
          value: selectionObject.name,
        });
      }
      if (
        selectionObject.attributes &&
        Array.isArray(selectionObject.attributes)
      ) {
  ...
  const itemPriceNum =
    itemPriceStr !== undefined && itemPriceStr !== null
      ? Number(itemPriceStr)
      : undefined;
  const quantityNum =
    quantity !== undefined && quantity !== null ? Number(quantity) : 1;
  const lineSubtotal =
    itemPriceNum !== undefined &&
    !isNaN(itemPriceNum) &&
...
const OrderArchiveDetail: React.FC = () => {
    ...
    return (
      <Box sx={{ p: 2 }}>
        <Button
          ...
          onClick={() => navigate("/admin/orderArchives/read")}
          sx={{ mb: 2 }}
        >
          Volver
        </Button>
  ...
  const createdByString =
    typeof orderData.createdBy === "string"
      ? orderData.createdBy
      : `${orderData.createdBy?.firstname || ""} ${orderData.createdBy?.lastname || ""} (${orderData.createdBy?.username || "Admin"})`.trim();

  const shippingMethodName =
    typeof orderData.shippingData.shippingMethod === "string"
      ? orderData.shippingData.shippingMethod
      : (orderData.shippingData.shippingMethod?.name ?? "N/A");

  const canUpdateOrderStatus = orderData
    ? !FINAL_ORDER_STATUSES_MODAL.includes(orderData.status) ||
      orderData.status === "Concretado"
    : false;
      ...
      <Grid2 container spacing={3}>
        ...
        <Grid2 size={{ xs: 12, md: 6 }}>
          <SectionCard title="Cliente" action={<PersonIcon color="action" />}>
            ...
            <InfoItem
              ...
              value={orderData.basicData?.address}
            ...
            {orderData.consumerData && (
              <>
                ...
                <InfoItem
                  label="ID Consumidor"
                  value={orderData.consumerData.consumerId}
                />
                <InfoItem
                  label="Tipo Consumidor"
                  value={orderData.consumerData.consumerType}
                />
              </>
            )}
          </SectionCard>
          ...
          <SectionCard
            title="Envío"
            action={<LocalShippingIcon color="action" />}
          >
            <InfoItem label="Método" value={shippingMethodName} />
            <InfoItem
              label="Recibe"
              value={
                `${orderData.shippingData.shippingName || orderData.shippingData.name || ""} ${orderData.shippingData.shippingLastName || orderData.shippingData.lastname || ""}`.trim() ||
                undefined
              }
            />
            <InfoItem
              label="Teléfono (Envío)"
              value={
                orderData.shippingData.shippingPhone ||
                orderData.shippingData.phone
              }
            />
            <InfoItem
              label="Dirección"
              value={
                orderData.shippingData.shippingAddress ||
            ...
            <InfoItem
              label="Ciudad/País"
              value={[
                orderData.shippingData.city,
                orderData.shippingData.country,
              ]
                .filter(Boolean)
                .join(", ")}
            />
            <InfoItem
              label="Código Postal"
              value={orderData.shippingData.zip}
            />
          </SectionCard>
        </Grid2>
        ...
        <Grid2 size={{ xs: 12, md: 6 }}>
          <SectionCard
            ...
            action={<ReceiptIcon color="action" />}
          >
            {orderData.billingData ? (
              <>
                <InfoItem
                  label="Facturar a"
                  value={
                    `${orderData.billingData.name || ""} ${orderData.billingData.lastname || ""}`.trim() ||
                    getCustomerName(orderData)
                  }
                />
                <InfoItem
                  label="CI/RIF (Fact.)"
                  value={orderData.billingData.ci}
                />
                <InfoItem
                  label="Empresa (Fact.)"
                  value={
                    orderData.billingData.billingCompany ||
                    orderData.billingData.company
                  }
                />
                <InfoItem
                  label="Teléfono (Fact.)"
                  value={orderData.billingData.phone}
                />
                <InfoItem
                  label="Dirección (Fact.)"
                  value={orderData.billingData.address}
                />
                <Divider sx={{ my: 1.5 }} />
                <InfoItem
                  label="Método de Pago"
                  value={orderData.billingData.orderPaymentMethod}
                  valueTypographyProps={{ fontWeight: "bold" }}
                />
              </>
            ) : (
              <Typography variant="body2" color="text.secondary">
                Sin datos de facturación específicos.
              </Typography>
            )}
            ...
            {orderData.dollarValue && (
              <Box sx={{ mt: 1.5 }}>
                <InfoItem
                  label="Valor Dólar (Referencia)"
                  value={"Bs " + orderData.dollarValue}
                />
              </Box>
            )}
          </SectionCard>
          ...
          >
            ...
            <InfoItem
              label="Impuesto (IVA)"
              value={formatCurrency(orderData.tax)}
            />
            <InfoItem
              label="Costo Envío"
              value={formatCurrency(orderData.shippingCost ?? 0)}
            />
            <Divider sx={{ my: 1 }} />
            <InfoItem
              label="Total Orden"
          ...
          <SectionCard
            ...
            action={<NotesIcon color="action" />}
          >
            <InfoItem
              label="Observaciones"
              value={orderData.observations}
              valueTypographyProps={{ whiteSpace: "pre-wrap" }}
            />
            <InfoItem label="Creado Por" value={createdByString} />
            {orderData.comissions && orderData.comissions.length > 0 && (
              <InfoItem
                label="Comisiones"
                value={`(${orderData.comissions.length}) Registradas`}
                valueTypographyProps={{ fontStyle: "italic" }}
              />
            )}
          </SectionCard>
        </Grid2>
        ...
        <Grid2 size={{ xs: 12 }}>
          <SectionCard
            title={`Items (${orderData.requests.length})`}
            action={<ShoppingCartIcon color="action" />}
          >
        ...
        >
          ...
          <DialogContent>
            ...
            <FormControl fullWidth margin="normal" disabled={isUpdatingStatus}>
              ...
              >
                {UPDATABLE_TARGET_ORDER_STATUSES_MODAL.filter(
            ...
            <FormControl fullWidth margin="normal" disabled={isUpdatingStatus}>
              ...
              >
                {ALL_PAY_STATUSES_MODAL.filter(
                  (ps) => ps !== orderData.payStatus,
                ).map((payStatusOption) => (
