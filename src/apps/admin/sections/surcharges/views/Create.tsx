...
interface SurchargeValidationErrors {
    ...
    recipients?: { [index: number]: { type?: string; id?: string; adjustmentMethod?: string; customValue?: string } }; // Add recipients errors
    summary?: string;
}

// --- Define helper type for single errors ---
type SingleRuleErrors = NonNullable<SurchargeValidationErrors['entityOverrides']>[number]; // Can reuse for recipients structure

// --- Component ---
const CreateSurcharge: React.FC = () => { // Rename component
    // --- Hooks ---
    ...
    const [validationErrors, setValidationErrors] = useState<SurchargeValidationErrors | null>(null); // Use new type
    ...
    const loadItems = useCallback(async () => {
        ...
        try {
            ...
            setAllProducts((await productsData).filter(p => p._id && p.variants && p.variants.length > 0));
        } catch (err: any) {
    ...
    const handleInputChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        if (validationErrors && validationErrors[name as keyof SurchargeValidationErrors]) {
    ...
    const handleRemoveOverride = (tempIdToRemove: string) => {
        ...
        const indexToRemove = formData.entityOverrides.findIndex((ov: OverrideFormState) => ov.tempId === tempIdToRemove);
    ...
    const handleAddRecipient = () => {
        setFormData(prev => ({
            ...prev,
            recipients: [...prev.recipients, { tempId: uuidv4(), type: 'user', adjustmentMethod: 'percentage', customValue: 0, id: '', name: '' }]
        }));
    ...
    const handleRecipientChange = (tempId: string, field: keyof ApplicableEntity, value: string | number | Entity | AdjustmentMethod) => {
        setFormData(prev => ({
            ...prev,
        ...
        if (validationErrors?.recipients) {
            const index = formData.recipients.findIndex((rec: RecipientFormState) => rec.tempId === tempId);
            if (index === -1) return;
            ...
            if (validationErrors.recipients[index]?.[errorFieldKey]) {
                setValidationErrors(prevErrors => {
                    ...
                    const finalRecipients = Object.keys(updatedRecipients).length > 0 ? updatedRecipients : undefined;
    ...
    const handleRemoveRecipient = (tempIdToRemove: string) => {
        // Find index *before* removing from state
        const indexToRemove = formData.recipients.findIndex((rec: RecipientFormState) => rec.tempId === tempIdToRemove);

        setFormData(prev => ({
            ...prev,
            recipients: prev.recipients.filter((rec: RecipientFormState) => rec.tempId !== tempIdToRemove)
        }));
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.appliesToAllProducts && formData.applicableProducts?.length === 0) { errors.applicableProducts = "Seleccione productos/variantes o marque 'Aplica a todos'."; errorMessages.push("Productos/Variantes"); }
        ...
        formData.entityOverrides.forEach((ov, index) => {
            ...
            if (Object.keys(currentErrors).length > 0) {
                ...
                if (!hasOverrideError) { errorMessages.push(`Excepción #${index + 1} (${overrideErrorFields.join(', ')})`); } // Add first override error group to summary
        ...
        formData.recipients.forEach((rec, index) => {
            ...
            if (Object.keys(currentErrors).length > 0) {
                ...
                if (!hasRecipientError) { errorMessages.push(`Destinatario #${index + 1} (${recipientErrorFields.join(', ')})`); } // Add first recipient error group to summary
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        // Clean recipients
        const finalRecipients: ApplicableEntity[] = formData.recipients.map((rec: RecipientFormState) => {
            const { tempId, ...rest } = rec;
            const recipientData: ApplicableEntity = {
                type: rest.type, adjustmentMethod: rest.adjustmentMethod,
                id: rest.id, // Keep ID (mandatory)
                name: rest.name?.trim() || undefined,
                customValue: Number(rest.customValue), // Ensure number
            };
            Object.keys(recipientData).forEach(key => { if (recipientData[key as keyof ApplicableEntity] === undefined) delete recipientData[key as keyof ApplicableEntity]; });
        ...
        const payload: Omit<Surcharge, '_id'> = {
            name: formData.name, description: formData.description, active: formData.active,
            adjustmentMethod: formData.adjustmentMethod, defaultValue: Number(formData.defaultValue),
            appliesToAllProducts: formData.appliesToAllProducts,
            applicableProducts: formData.appliesToAllProducts ? [] : formData.applicableProducts, // Correct format
            appliestoAllArts: false,
        ...
        try {
            ...
            } else { throw new Error("La creación del recargo falló (respuesta vacía)."); }
        } catch (err: any) {
            console.error("Failed to create surcharge:", err);
            const message = err?.response?.data?.message || err.message || "Error al crear el recargo.";
    ...
    return (
        <>
            ...
            {/* --- End Title Update --- */}
            <Paper elevation={3} sx={{ p: { xs: 2, sm: 3 }, mt: 2 }}> {/* Adjusted padding */}
                <form onSubmit={handleSubmit} noValidate>
                    ...
                    <Grid2 container spacing={3}>
                        ...
                        <Grid2 size={{ xs: 12 }}><Divider sx={{ my: 2 }} /></Grid2> {/* Divider */}
                        ...
                        <Grid2 size={{ xs: 12, sm: 6 }}>
                            <TextField
                                ...
                                name="defaultValue" type="number" value={formData.defaultValue} onChange={handleInputChange} required fullWidth disabled={isSubmitting}
                        ...
                        <Grid2 size={{ xs: 12 }}>
                            ...
                            >
                                ...
                                <AccordionDetails>
                                    <Grid2 container spacing={2}>
                                        ...
                                        <Grid2 size={{ xs: 12 }}>
                                            <Box sx={{ border: 1, borderColor: (theme) => (validationErrors?.applicableProducts ? theme.palette.error.main : 'divider'), borderRadius: 1, p: 2 }}>
                                    ...
                                    <Grid2 container spacing={2}>
                                        <Grid2 size={{ xs: 12, sm: 6 }}>
                                            <DatePicker
                                                ...
                                                disabled={isSubmitting}
                                                slotProps={{ textField: { fullWidth: true, error: !!validationErrors?.startDate || !!validationErrors?.dateRange, helperText: validationErrors?.startDate || validationErrors?.dateRange } }}
                                            />
                                        </Grid2>
                                        <Grid2 size={{ xs: 12, sm: 6 }}>
                                            <DatePicker
                                                ...
                                                slotProps={{ textField: { fullWidth: true, error: !!validationErrors?.endDate || !!validationErrors?.dateRange, helperText: validationErrors?.endDate || validationErrors?.dateRange } }}
                                            />
                                        </Grid2>
                                    </Grid2>
                                </AccordionDetails>
                                        ...
                                        return (
                                            <Box key={override.tempId} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, p: 2, mb: 2, position: 'relative' }}>
                                                <Tooltip title="Eliminar Excepción">
                                                    <IconButton size="small" onClick={() => handleRemoveOverride(override.tempId)} disabled={isSubmitting} sx={{ position: 'absolute', top: 8, right: 8, color: 'error.main' }} aria-label="Eliminar Excepción">
                                                ...
                                                <Grid2 container spacing={2}>
                                                    ...
                                                    <Grid2 size={{xs:12, sm:6, md:3}}>
                                                        <TextField
                                                            ...
                                                            InputProps={{ inputProps: override.adjustmentMethod === 'percentage' ? { step: "0.01", min: "0", max: "1" } : { step: "0.01", min: "0" } }} />
                                                    </Grid2>
                                                </Grid2>
                                            </Box>
                                        );
                            ...
                            <Accordion expanded={expandedAccordion === 'recipients'} onChange={handleAccordionChange('recipients')} sx={{ '&.Mui-expanded': { margin: '16px 0' } }}>
                                <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="recipients-content" id="recipients-header">
                                    <Typography variant="h6" component="div">Destinatarios del Recargo (Opcional)</Typography>
                                </AccordionSummary>
                                <AccordionDetails>
                                    {formData.recipients.length === 0 && (
                                        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>No se han añadido destinatarios. Si no se añaden, el recargo no se distribuirá.</Typography>
                                    )}
                                    {formData.recipients.map((recipient: RecipientFormState, index) => {
                                        const recipientErrors = validationErrors?.recipients?.[index] || {};
                                        return (
                                            <Box key={recipient.tempId} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, p: 2, mb: 2, position: 'relative' }}>
                                                <Tooltip title="Eliminar Destinatario">
                                                    <IconButton size="small" onClick={() => handleRemoveRecipient(recipient.tempId)} disabled={isSubmitting} sx={{ position: 'absolute', top: 8, right: 8, color: 'error.main' }} aria-label="Eliminar Destinatario">
                                                        <DeleteIcon fontSize="small" />
                                                    </IconButton>
                                                </Tooltip>
                                                <Grid2 container spacing={2}>
                                                    {/* Type */}
                                                    <Grid2 size={{xs:12, sm:6, md:3}}>
                                                        <FormControl fullWidth required error={!!recipientErrors.type}>
                                                            <InputLabel id={`recipient-type-label-${index}`}>Tipo Entidad</InputLabel>
                                                            <Select
                                                                labelId={`recipient-type-label-${index}`}
                                                                label="Tipo Entidad" // Added label prop
                                                                name="type"
                                                                value={recipient.type}
                                                                onChange={(e) => handleRecipientChange(recipient.tempId, 'type', e.target.value as Entity)}
                                                                disabled={isSubmitting}
                                                            >
                                                                {ENTITY_TYPE_OPTIONS.map(opt => (
                                                                    <MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>
                                                            ...
                                                            <FormHelperText>{recipientErrors.type}</FormHelperText>
                                                        </FormControl>
                                                    </Grid2>
                                                    ...
                                                    <Grid2 size={{xs:12, sm:6, md:3}}>
                                                        <TextField
                                                            ...
                                                            name="id"
                                                            value={recipient.id || ""}
                                                            onChange={(e) => handleRecipientChange(recipient.tempId, 'id', e.target.value)}
                                                            fullWidth
                                                            required // Make ID required
                                                            disabled={isSubmitting}
                                                            error={!!recipientErrors.id}
                                                            helperText={recipientErrors.id || "ID del destinatario es obligatorio"}
                                                        />
                                                    </Grid2>
                                                    {/* Method */}
                                                    <Grid2 size={{xs:12, sm:6, md:3}}>
                                                        <FormControl fullWidth required error={!!recipientErrors.adjustmentMethod}>
                                                            <InputLabel id={`recipient-method-label-${index}`}>Método Dest.</InputLabel>
                                                            <Select
                                                                labelId={`recipient-method-label-${index}`}
                                                                label="Método Dest." // Added label prop
                                                                name="adjustmentMethod"
                                                                value={recipient.adjustmentMethod}
                                                                onChange={(e) => handleRecipientChange(recipient.tempId, 'adjustmentMethod', e.target.value as AdjustmentMethod)}
                                                                disabled={isSubmitting}
                                                            >
                                                                {ADJUSTMENT_METHOD_OPTIONS.map(opt => (
                                                                    <MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>
                                                            ...
                                                            <FormHelperText>{recipientErrors.adjustmentMethod}</FormHelperText>
                                                        </FormControl>
                                                    </Grid2>
                                                    {/* Value */}
                                                    <Grid2 size={{xs:12, sm:6, md:3}}>
                                                        <TextField
                                                            ...
                                                            name="customValue" type="number" value={recipient.customValue ?? ''}
                                                            onChange={(e) => handleRecipientChange(recipient.tempId, 'customValue', e.target.value)}
                                                            required fullWidth disabled={isSubmitting} error={!!recipientErrors.customValue}
                                                            helperText={recipientErrors.customValue || (recipient.adjustmentMethod === 'percentage' ? "Ej: 0.10 para 10%" : "Ej: 2.50")}
                                                            InputProps={{ inputProps: recipient.adjustmentMethod === 'percentage' ? { step: "0.01", min: "0", max: "1" } : { step: "0.01", min: "0" } }} />
                                                    </Grid2>
                                                </Grid2>
                                            </Box>
                                        );
                                    })}
                                    <Button startIcon={<AddCircleOutlineIcon />} onClick={handleAddRecipient} disabled={isSubmitting} variant="outlined" size="small" sx={{ mt: formData.recipients.length > 0 ? 0 : 2 }}>
                                        Añadir Destinatario
                                    </Button>
                                </AccordionDetails>
                            </Accordion>
