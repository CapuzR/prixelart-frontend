...
const CreateArt: React.FC = () => {
    ...
    const [validationErrors, setValidationErrors] = useState<ArtValidationErrors | null>(null); // Use object for specific errors
    ...
    // --- Fetch Prixers ---
    const loadPrixers = useCallback(async () => { /* ... same as before ... */
        ...
        try {
            ...
            console.error("Error loading prixers:", err); showSnackBar("Error al cargar Prixers.");
        } finally { setIsLoadingPrixers(false); }
    }, [showSnackBar]);

    useEffect(() => { loadPrixers(); }, [loadPrixers]);
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!validateForm() || !selectedPrixer?._id) {
            if (!selectedPrixer?._id && !validationErrors?.prixer) {
                ...
                setValidationErrors(prev => ({ ...prev, prixer: "Error interno: ID del Prixer no encontrado." }));
    ...
    return (
        <>
            ...
            <Paper elevation={3} sx={{ p: 3, mt: 2 }}>
                <form onSubmit={handleSubmit} noValidate> {/* Use noValidate with manual validation */}
                    <Grid2 container spacing={3}>
                        {/* Title */}
                        <Grid2 size={{ xs: 12, sm: 6 }}>
                            <TextField label="Título del Arte" name="title" value={formData.title} onChange={handleInputChange} required fullWidth disabled={isSubmitting} error={!!validationErrors?.title} helperText={validationErrors?.title} />
                        </Grid2>

                        {/* Owner (Prixer) Selection */}
                        <Grid2 size={{ xs: 12, sm: 6 }}>
                            <Autocomplete
                                id="prixer-owner-select" options={prixerOptions} value={selectedPrixer} onChange={handlePrixerChange} getOptionLabel={(option) => option.label} isOptionEqualToValue={(option, value) => option.username === value.username} loading={isLoadingPrixers} disabled={isSubmitting}
                                ...
                                renderInput={(params) => (<TextField {...params} label="Propietario (Prixer)" required error={!!validationErrors?.prixer} helperText={validationErrors?.prixer} InputProps={{ ...params.InputProps, endAdornment: (<>{isLoadingPrixers ? <CircularProgress color="inherit" size={20} /> : null}{params.InputProps.endAdornment}</>), }} />)}
                            />
                        </Grid2>
                        ...
                        <Grid2 size={{ xs: 12 }}>
                            ...
                            {/* Image Preview */}
                            {formData.imageUrl && !validationErrors?.imageUrl && ( // Only show preview if URL is present and seems valid
                                <Box sx={{ mt: 1, p: 1, border: 1, borderColor: 'divider', borderRadius: 1, maxWidth: 250, maxHeight: 250, display: 'inline-block' }}>
                                    <img src={formData.imageUrl} alt="Previsualización" style={{ display: 'block', width: '100%', height: 'auto', objectFit: 'contain', maxHeight: 230 }} onError={(e) => (e.currentTarget.style.display = 'none')} onLoad={(e) => (e.currentTarget.style.display = 'block')} />
                                </Box>
                            )}
                        </Grid2>
                        ...
                        {/* Description */}
                        <Grid2 size={{ xs: 12 }}>
                            <TextField label="Descripción" name="description" value={formData.description} onChange={handleInputChange} required fullWidth multiline rows={4} disabled={isSubmitting} error={!!validationErrors?.description} helperText={validationErrors?.description} />
                        </Grid2>

                        {/* Art Type & Location */}
                        <Grid2 size={{ xs: 12, sm: 6 }}>
                            <TextField label="Tipo de Arte (Ej: Fotografía)" name="artType" value={formData.artType} onChange={handleInputChange} fullWidth disabled={isSubmitting} error={!!validationErrors?.artType} helperText={validationErrors?.artType} />
                        </Grid2>
                        <Grid2 size={{ xs: 12, sm: 6 }}>
                            <TextField label="Ubicación del Arte (Ej: Galería)" name="artLocation" value={formData.artLocation} onChange={handleInputChange} fullWidth disabled={isSubmitting} error={!!validationErrors?.artLocation} helperText={validationErrors?.artLocation} />
                        </Grid2>


                        {/* Category */}
                        <Grid2 size={{ xs: 12, sm: 6 }}>
                            <TextField label="Categoría" name="category" value={formData.category} onChange={handleInputChange} fullWidth disabled={isSubmitting} error={!!validationErrors?.category} helperText={validationErrors?.category} />
                        </Grid2>

                        {/* Commission */}
                        <Grid2 size={{ xs: 12, sm: 6 }}>
                            <TextField label="Comisión (Ej: 0.15 para 15%)" name="comission" type="number" value={formData.comission} onChange={handleInputChange} required fullWidth disabled={isSubmitting} inputProps={{ step: "0.01", min: "0", max: "1" }} error={!!validationErrors?.comission} helperText={validationErrors?.comission} />
                        </Grid2>

                        {/* Tags Input (Using Autocomplete) */}
                        <Grid2 size={{ xs: 12 }}>
                            <Autocomplete
                                multiple freeSolo options={[]} // No predefined suggestions in this example
                        ...
                        <Grid2 size={{ xs: 12 }} sx={{ display: 'flex', alignItems: 'center' }}>
                            <FormControlLabel control={<Checkbox checked={formData.visible} onChange={handleInputChange} name="visible" disabled={isSubmitting} />} label="Visible en Tienda" />
                        </Grid2>
                        ...
                        <Grid2 size={{ xs: 12 }}>
                            <Stack direction="row" justifyContent="flex-end" spacing={2} sx={{ mt: 2 }}>
                                ...
                                <Button type="submit" variant="contained" color="primary" disabled={isSubmitting || isLoadingPrixers} startIcon={isSubmitting ? <CircularProgress size={20} color="inherit" /> : null}>
                        ...
                        {validationErrors && !validationErrors.title && !validationErrors.description /* etc */ && validationErrors.prixer?.includes("Error interno") && (
                            <Grid2 size={{ xs: 12 }}>
                                <Alert severity="error" sx={{ mt: 2 }}>{validationErrors.prixer}</Alert>
                            </Grid2>
                        )}
                        {validationErrors && validationErrors.title && validationErrors.title !== "El título es obligatorio." && !validationErrors.description /* etc */ && ( // Crude check for general API error shown near title
                            <Grid2 size={{ xs: 12 }}>
                                <Alert severity="error" sx={{ mt: 2 }}>{validationErrors.title}</Alert>
                            </Grid2>
                        )}
