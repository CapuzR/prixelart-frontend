...
import {
    ...
    Tooltip, Link, Chip, TablePagination, TableSortLabel,
    FormControl, Select, TextField, InputLabel, MenuItem,
    Skeleton, Typography, Stack, ToggleButton,
    SelectChangeEvent, InputAdornment,
    DialogActions,
    DialogContent,
    DialogTitle,
    Dialog,
...
import { OrderArchive, Status as OrderStatus, PayStatus as OrderPayStatus } from 'types/orderArchive.types';
...
// --- Constants ---
const ALL_STATUSES: OrderStatus[] = ["Anulado", "Concretado", "En producción", "Por entregar", "Entregado", "En impresión", "Por producir"];
const ALL_PAY_STATUSES: OrderPayStatus[] = ["Pagado", "Anulado", "Obsequio", "Abonado", "Pendiente"];
const FINAL_STATUSES: OrderStatus[] = ["Concretado", "Entregado", "Anulado"];
const UPDATABLE_TARGET_STATUSES: OrderStatus[] = ALL_STATUSES.filter(s => !FINAL_STATUSES.includes(s) || s === "Concretado");
...
type OrderSortKey = keyof Pick<OrderArchive, 'orderId' | 'createdOn' | 'orderType' | 'status' | 'payStatus' | 'total'>;
...
const ReadOrderArchives: React.FC = () => {
    ...
    const [rowsPerPage, setRowsPerPage] = useState<number>(() => parseInt(searchParams.get('limit') || '10', 10));
    const [totalOrders, setTotalOrders] = useState<number>(0);

    // --- Sorting State ---
    const [orderBy, setOrderBy] = useState<OrderSortKey>(() => (searchParams.get('sortBy') as OrderSortKey) || 'createdOn');
    const [order, setOrder] = useState<SortDirection>(() => (searchParams.get('sortOrder') as SortDirection) || 'desc');

    // --- Filtering & Search State ---
    const [searchQuery, setSearchQuery] = useState<string>(() => searchParams.get('search') || '');
    const [filterStatus, setFilterStatus] = useState<OrderStatus | ''>(() => (searchParams.get('status') as OrderStatus) || '');
    const [filterPayStatus, setFilterPayStatus] = useState<OrderPayStatus | ''>(() => (searchParams.get('payStatus') as OrderPayStatus) || '');
    const [startDate, setStartDate] = useState<Date | null>(() => parseDateParam(searchParams.get('startDate')));
    const [endDate, setEndDate] = useState<Date | null>(() => parseDateParam(searchParams.get('endDate')));
    const [filterIsNotConcretado, setFilterIsNotConcretado] = useState<boolean>(() => searchParams.get('excludeStatus') === 'Concretado');

    // --- Update Status Modal State ---
    const [isUpdateModalOpen, setIsUpdateModalOpen] = useState<boolean>(false);
    const [selectedOrderForUpdate, setSelectedOrderForUpdate] = useState<OrderArchive | null>(null);
    ...
    // --- Data Fetching ---
    const loadOrders = useCallback(async (showInitialLoadingIndicator = true) => {
        const loadingStateSetter = showInitialLoadingIndicator ? setIsLoading : setIsFilteringLoading;
    ...
    }, [page, rowsPerPage, order, orderBy, showSnackBar, searchQuery, filterStatus, filterPayStatus, startDate, endDate, filterIsNotConcretado]);
    ...
    useEffect(() => {
        if (!isInitialMount.current) {
            ...
            if (startDate && isValid(startDate)) params.startDate = format(startDate, 'yyyy-MM-dd');
            if (endDate && isValid(endDate)) params.endDate = format(endDate, 'yyyy-MM-dd');
    ...
    }, [page, rowsPerPage, order, orderBy, searchQuery, filterStatus, filterPayStatus, startDate, endDate, filterIsNotConcretado, setSearchParams]);
    ...
    // ADDED: Handler for pay status change in modal
    ...
    const handleConfirmUpdateStatus = async () => {
        ...
        const statusChanged = newStatus && newStatus !== selectedOrderForUpdate.status;
        const payStatusChanged = newPayStatus && newPayStatus !== selectedOrderForUpdate.payStatus;
        ...
        if (statusChanged && newStatus) { // ensure newStatus is not empty
            payload.status = newStatus;
        }
        if (payStatusChanged && newPayStatus) { // ensure newPayStatus is not empty
        ...
        try {
            ...
            setError(`Error actualizando orden ${selectedOrderForUpdate.orderId}: ${message}`);
    ...
    const handleRequestSort = (event: React.MouseEvent<unknown>, property: OrderSortKey) => {
    ...
    const handleSelectFilterChange = (setter: React.Dispatch<React.SetStateAction<any>>) => (event: SelectChangeEvent<any>) => {
        const value = event.target.value;
        setter(value);
    ...
    const handleDateFilterChange = (setter: React.Dispatch<React.SetStateAction<Date | null>>) =>
    ...
    interface EnhancedTableProps {
        onRequestSort: (event: React.MouseEvent<unknown>, property: OrderSortKey) => void;
        order: SortDirection;
        orderBy: string;
    }
    ...
    function EnhancedTableHead(props: EnhancedTableProps) {
        ...
        const createSortHandler = (property: OrderSortKey) => (event: React.MouseEvent<unknown>) => {
                ...
                <TableRow>
                    {headCells.map((headCell) => (
                        <TableCell
                            ...
                            sx={{ fontWeight: 'bold', display: headCell.display, width: headCell.width }}
                        >
                            {headCell.sortable ? (
                                <TableSortLabel
    ...
    const renderActiveFilters = () => {
        ...
        if (searchQuery) {
            activeFilters.push(<Chip key="search" label={`Buscar: ${searchQuery}`} size="small" onDelete={() => { setSearchQuery(''); triggerSearchOrFilter(); }} />);
        }
        if (filterIsNotConcretado) {
            activeFilters.push(<Chip key="not_concretado" label="Status: Pendientes" size="small" onDelete={handleNotConcretadoToggle} />);
        }
        if (filterStatus) {
            activeFilters.push(<Chip key="status" label={`Status: ${filterStatus}`} size="small" onDelete={() => { setFilterStatus(''); triggerSearchOrFilter(); }} />);
        }
        if (filterPayStatus) {
            activeFilters.push(<Chip key="payStatus" label={`Pago: ${filterPayStatus}`} size="small" onDelete={() => { setFilterPayStatus(''); triggerSearchOrFilter(); }} />);
        }
        if (startDate && isValid(startDate)) {
            activeFilters.push(<Chip key="startDate" label={`Desde: ${format(startDate, 'dd/MM/yyyy')}`} size="small" onDelete={() => { setStartDate(null); triggerSearchOrFilter(); }} />);
        }
        if (endDate && isValid(endDate)) {
            activeFilters.push(<Chip key="endDate" label={`Hasta: ${format(endDate, 'dd/MM/yyyy')}`} size="small" onDelete={() => { setEndDate(null); triggerSearchOrFilter(); }} />);
        }
        ...
        return (
            <Stack direction="row" spacing={1} sx={{ p: 1, flexWrap: 'wrap', gap: 1 }}>
                <Typography variant="caption" sx={{ alignSelf: 'center', mr: 1 }}>Filtros Activos:</Typography>
                {activeFilters}
            </Stack>
        );
    };
    ...
    const renderContent = () => {
        ...
        if (error && orders.length === 0 && !isLoading) {
            return (<Alert severity="error" sx={{ m: 2 }}>{error}<Button onClick={() => loadOrders(true)} size="small">Reintentar</Button></Alert>);
        }

        if (!isLoading && !isFilteringLoading && orders.length === 0 && !error) {
            return (<Alert severity="info" sx={{ m: 2 }}>No se encontraron órdenes con los filtros aplicados.</Alert>);
        }
        ...
        return (
            <Paper sx={{ width: '100%', mb: 2, overflow: 'hidden', position: 'relative' }}>
                ...
                <TableContainer sx={{ maxHeight: 600 }}>
                    <Table stickyHeader sx={{ minWidth: 750 }} aria-label="orders table" size="small">
                        <EnhancedTableHead
                            order={order}
                            orderBy={orderBy}
                            ...
                            {showSkeletons
                                ? Array.from(new Array(rowsPerPage)).map((_, index) => (
                                    <TableRow key={`skeleton-${index}`} sx={{ height: 33 }}>
                                        {headCells.map((cell) => (
                                            <TableCell key={cell.id} align={cell.numeric ? 'right' : 'left'} sx={{ display: cell.display, width: cell.width }}>
                                ...
                                : orders.map((orderItem) => {
                                    ...
                                    const isUpdatable = !FINAL_STATUSES.includes(orderItem.status);
                                    ...
                                    return (
                                        <TableRow
                                            ...
                                            onClick={!isUpdatingStatus ? () => handleViewDetail(orderItem._id.toString()) : undefined}
                                            sx={{
                                        ...
                                        >
                                            <TableCell>
                                                <Link component="button" variant="body2" onClick={(e) => { e.stopPropagation(); handleViewDetail(orderItem._id.toString()); }} sx={{ textAlign: 'left' }}>
                                            ...
                                            <TableCell sx={{ display: headCells.find(h => h.id === 'orderType')?.display }}>{orderItem.orderType || '-'}</TableCell>
                                            <TableCell>
                                                {orderItem.status ? (
                                                    <Chip label={orderItem.status} color={getStatusColor(orderItem.status)} size="small" />
                                                ) : '-'}
                                            </TableCell>
                                            <TableCell sx={{ display: headCells.find(h => h.id === 'payStatus')?.display }}>
                                                {orderItem.payStatus ? (
                                                ...
                                                <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.5 }} onClick={(e) => e.stopPropagation()}>
                                                    ...
                                                    <Tooltip title="Ver Detalles de Orden">
                                                        <IconButton
                                                            ...
                                                            color="primary"
                                                            onClick={() => handleViewDetail(orderItem._id.toString())}
                                                            disabled={!orderItem._id || isLoading || isFilteringLoading} // Disable if loading
                                                            size="small"
                                                        >
                                                    ...
                                                    <Tooltip title={isUpdatable ? "Actualizar Status" : `No se puede actualizar (Status: ${orderItem.status})`}>
                                                        ...
                                                        <span>
                                                            <IconButton
                                                                ...
                                                                color="secondary"
                                                                onClick={() => handleOpenUpdateModal(orderItem)}
                                                                disabled={!isUpdatable || isLoading || isFilteringLoading || isUpdatingStatus} // Disable based on status or loading states
                                                                size="small"
                                                            >
            ...
            <>
                ...
                <Paper sx={{ p: 2, mb: 2 }}>
                    <Grid2 container spacing={2} alignItems="center">
                        ...
                        <Grid2 size={{ xs: 6, sm: 3, md: 2, lg: 1.5 }}>
                            <Tooltip title={filterIsNotConcretado ? "Mostrar todas las ordenes" : "Mostrar solo órdenes pendientes"}>
                                <span>
                                    <ToggleButton
                                        value="check"
                                        ...
                                        sx={{ height: '40px' }}
                                    >
                                        {filterIsNotConcretado ? 'Mostrar todas' : 'Mostrar Solo Pendientes'}
                                    </ToggleButton>
                                </span>
                            </Tooltip>
                        </Grid2>
                        {/* Status Filter */}
                        <Grid2 size={{ xs: 6, sm: 3, md: 2, lg: 1.5 }}>
                            <FormControl fullWidth size="small" disabled={isFilteringLoading || filterIsNotConcretado}>
                                ...
                                >
                                    ...
                                    {ALL_STATUSES.map(s => (
                                        <MenuItem key={s} value={s} disabled={filterIsNotConcretado}>{s}</MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                        </Grid2>
                        ...
                        <Grid2 size={{ xs: 12, sm: 12, md: 12, lg: 1 }} container justifyContent="flex-end" alignItems="center">
                            <Tooltip title="Limpiar Filtros">
                                <span>
                                    <IconButton
                                        onClick={handleClearFilters}
                                        disabled={!searchQuery && !filterStatus && !filterPayStatus && !startDate && !endDate && !filterIsNotConcretado || isFilteringLoading}
                                        color="default"
                                    >
                                        <FilterListOffIcon />
                                    </IconButton>
                ...
                {error && !isLoading && !isFilteringLoading && orders.length > 0 && (
                    <Alert severity="warning" sx={{ mb: 2 }}>
                        ...
                        <Button onClick={() => loadOrders(false)} size="small" sx={{ ml: 1 }}>Reintentar</Button>
                    </Alert>
                )}
                {renderContent()}
                ...
                <Dialog open={isUpdateModalOpen} onClose={handleCloseUpdateModal} aria-labelledby="update-status-dialog-title" fullWidth maxWidth="xs">
                    ...
                    <DialogContent>
                        <DialogContentText sx={{ mb: 2 }}>
                            Orden ID: <strong>{selectedOrderForUpdate?.orderId}</strong> <br />
                            Status Actual: <Chip label={selectedOrderForUpdate?.status} color={getStatusColor(selectedOrderForUpdate?.status)} size="small" /> <br />
                            Status Pago Actual: <Chip label={selectedOrderForUpdate?.payStatus || 'N/A'} color={getPayStatusColor(selectedOrderForUpdate?.payStatus)} size="small" />
                        </DialogContentText>

                        {/* Order Status Selector */}
                        <FormControl fullWidth margin="normal" disabled={isUpdatingStatus || (selectedOrderForUpdate ? (FINAL_STATUSES.includes(selectedOrderForUpdate.status) && selectedOrderForUpdate.status !== "Concretado") : false)}>
                            ...
                            >
                                {UPDATABLE_TARGET_STATUSES
                                    .filter(s => s !== selectedOrderForUpdate?.status)
                                    .map(statusOption => (
                                        <MenuItem key={statusOption} value={statusOption}>{statusOption}</MenuItem>
                                    ))}
                            </Select>
                            {(selectedOrderForUpdate && FINAL_STATUSES.includes(selectedOrderForUpdate.status) && selectedOrderForUpdate.status !== "Concretado") && (
                                <Typography variant="caption" color="textSecondary" sx={{ mt: 0.5 }}>
                                    El status de la orden no se puede cambiar desde '{selectedOrderForUpdate.status}'.
                                </Typography>
                            )}
                        </FormControl>
                        ...
                        <FormControl fullWidth margin="normal" disabled={isUpdatingStatus}>
                            ...
                            >
                                {ALL_PAY_STATUSES
                                    .filter(ps => ps !== selectedOrderForUpdate?.payStatus)
                                    .map(payStatusOption => (
