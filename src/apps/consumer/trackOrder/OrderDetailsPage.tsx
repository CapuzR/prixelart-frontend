...
import {
    ...
    List, ListItem, ListItemText, Divider, Card, CardContent, Avatar, Chip, Button, Icon, Link as MuiLink,
    ListItemIcon
} from '@mui/material';
import Grid2 from '@mui/material/Grid';
import {
    CalendarToday, PersonOutline, MailOutline, PhoneOutlined, HomeOutlined,
    LocalShippingOutlined, ReceiptOutlined, StorefrontOutlined, InfoOutlined, ArrowBack,
    PaletteOutlined,
    CollectionsOutlined
...
import { VariantAttribute } from 'types/product.types';
...
const OrderDetailsPage: React.FC = () => {
    ...
    const renderInfoItem = (icon: React.ReactNode, primary: string, secondary: string | React.ReactNode | undefined) => (
        secondary ? (
            <ListItem sx={{ py: 0.5 }}>
                ...
                <ListItemText primary={primary} secondary={secondary} primaryTypographyProps={{ variant: 'body2', fontWeight: 'medium' }} secondaryTypographyProps={{ variant: 'body2', color: 'text.secondary' }} />
            </ListItem>
        ) : null
    );
    ...
    if (loading) {
        return (
            <Container sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
                ...
                <Typography variant="h6" sx={{ mt: 3, color: 'text.secondary' }}>Cargando detalles de la orden...</Typography>
            </Container>
        );
    }
        ...
        return (
            <Container sx={{ mt: { xs: 2, md: 4 }, textAlign: 'center' }}>
                <Paper elevation={3} sx={{ p: 4, maxWidth: '600px', margin: 'auto' }}>
                    <Alert severity="error" icon={<InfoOutlined sx={{ fontSize: '2rem' }} />} sx={{ mb: 3, justifyContent: 'center', '& .MuiAlert-message': { textAlign: 'center' } }}>
                        <Typography variant="h6" gutterBottom>No se pudo cargar la orden</Typography>
                        {error}
                    </Alert>
                    <Button variant="outlined" startIcon={<ArrowBack />} onClick={() => navigate('/track')}>
                        Volver a rastrear
                    </Button>
                </Paper>
            </Container>
    ...
    if (!order) {
        return (
            <Container sx={{ mt: 4, textAlign: 'center' }}>
                <Alert severity="warning">No se encontró información para la orden ID: {orderId}.</Alert>
                <Button variant="outlined" startIcon={<ArrowBack />} sx={{ mt: 2 }} onClick={() => navigate('/track')}>
    ...
    return (
        <Container maxWidth="lg" sx={{ py: { xs: 2, md: 4 } }}>
            <Button component={RouterLink} to="/track" startIcon={<ArrowBack />} sx={{ mb: 2 }}>
                Rastrear otra orden
            </Button>
            <Paper elevation={2} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3, mb: 4 }}>
                ...
                {order.shipping.estimatedDeliveryDate && overallStatus !== OrderStatus.Delivered && overallStatus !== OrderStatus.Canceled && (
                    <Alert severity="info" icon={<LocalShippingOutlined />} sx={{ mb: 3, borderRadius: 2 }}>
                        Fecha estimada de entrega: <strong>{formatDate(order.shipping.estimatedDeliveryDate, false)}</strong>
                    </Alert>
                )}
            </Paper>
            ...
            <Grid2 container spacing={{ xs: 2, md: 4 }}>
                            ...
                            <Card key={line.id} sx={{ mb: 3, borderRadius: 3, boxShadow: 3 }}>
                                <CardContent sx={{ p: { xs: 2, md: 3 } }}>
                                    <Grid2 container spacing={2} alignItems="center">
                                        <Grid2 size={{ xs: 12, sm: 2 }} sx={{ textAlign: 'center' }}>
                                            <Avatar
                                                variant="rounded"
                                                src={line.item.product.sources.images[0]?.url || 'https://via.placeholder.com/80?text=Img'}
                                                alt={line.item.product.name}
                                                sx={{ width: { xs: 60, sm: 80 }, height: { xs: 60, sm: 80 }, m: 'auto', borderRadius: 1.5 }}
                                            />
                                        </Grid2>
                                        <Grid2 size={{ xs: 12, sm: 6 }}>
                                            <Typography variant="h6" component="div" fontWeight="medium" sx={{ fontSize: { xs: '1.1rem', sm: '1.25rem' } }}>{line.item.product.name}</Typography>
                                            <Typography variant="body2" color="textSecondary">SKU: {line.item.sku}</Typography>
                                            <Typography variant="body2" color="textSecondary">Cantidad: {line.quantity} x ${parseFloat(line.item.price).toFixed(2)}</Typography>
                                        ...
                                        <Timeline position="right" sx={{ p: 0 }}>
                                            {line.status
                                                ...
                                                .map(([statusEnum, date], idx) => (
                                                    <TimelineItem key={idx} sx={{ minHeight: 56, '&::before': { content: 'none' } }} > {/* Adjusted TimelineItem for better alignment */}
                                                        <TimelineOppositeContent sx={{ flex: 0.3, pr: 1, textAlign: 'left' }}> {/* Adjusted flex and padding */}
                                                            <Typography variant="caption" color="text.secondary">
                                                                {formatDate(date, true)} {/* Ensure time is included if needed */}
                                                            </Typography>
                                                        ...
                                                        <TimelineSeparator>
                                                            ...
                                                            {idx < line.status.length - 1 && <TimelineConnector />}
                                                        </TimelineSeparator>
                                                        <TimelineContent sx={{ pl: 2 }}> {/* Added padding to content */}
                ...
                <Grid2 size={{ xs: 12, lg: 5 }} >
                    <Paper elevation={1} sx={{ p: { xs: 2, md: 3 }, mb: 3, borderRadius: 3 }}>
                        ...
                        <List dense disablePadding>
                            {renderInfoItem(null, "Subtotal:", `$${order.subTotal.toFixed(2)}`)}
                            {order.discount ? renderInfoItem(null, "Descuento:", <Typography color="success.main">-${order.discount.toFixed(2)}</Typography>) : null}
                            {order.shippingCost ? renderInfoItem(null, "Costo de Envío:", `$${order.shippingCost.toFixed(2)}`) : null}
                            {order.tax.map(t => renderInfoItem(null, `${t.name} (${t.value}%):`, `$${t.amount.toFixed(2)}`))}
                            <Divider sx={{ my: 1.5 }} />
                            <ListItem sx={{ py: 1 }}>
                    ...
                    {order.shipping && (
                        <Paper elevation={1} sx={{ p: { xs: 2, md: 3 }, mb: 3, borderRadius: 3 }}>
                            <Typography variant="h6" gutterBottom sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
                                <LocalShippingOutlined sx={{ mr: 1, color: 'primary.main' }} /> Detalles de Envío
                            </Typography>
                            <List dense disablePadding>
                                {renderInfoItem(<Icon component={LocalShippingOutlined} fontSize="small" />, "Método:", order.shipping.method.name)}
                                {renderInfoItem(<HomeOutlined fontSize="small" />, "Dirección:", formatAddress(order.shipping.address.address))}
                                {order.shipping.address.recepient.name && renderInfoItem(<PersonOutline fontSize="small" />, "Recibe:", `${order.shipping.address.recepient.name} ${order.shipping.address.recepient.lastName}`)}
                                {order.shipping.address.recepient.phone && renderInfoItem(<PhoneOutlined fontSize="small" />, "Teléfono:", order.shipping.address.recepient.phone)}
                            </List>
                        </Paper>
                    )}
                    ...
                    {order.consumerDetails && (
                        <Paper elevation={1} sx={{ p: { xs: 2, md: 3 }, mb: 3, borderRadius: 3 }}>
                            ...
                            <List dense disablePadding>
                                {renderInfoItem(<PersonOutline fontSize="small" />, "Nombre:", `${order.consumerDetails.basic.name} ${order.consumerDetails.basic.lastName}`)}
                                {order.consumerDetails.basic.email && renderInfoItem(<MailOutline fontSize="small" />, "Email:", <MuiLink href={`mailto:${order.consumerDetails.basic.email}`}>{order.consumerDetails.basic.email}</MuiLink>)}
                                {order.consumerDetails.basic.phone && renderInfoItem(<PhoneOutlined fontSize="small" />, "Teléfono:", <MuiLink href={`tel:${order.consumerDetails.basic.phone}`}>{order.consumerDetails.basic.phone}</MuiLink>)}
                            </List>
                        </Paper>
                    )}
                    {order.seller && (
                    ...
                    {order.observations && (
                        <Paper elevation={1} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}>
                            <Typography variant="h6" gutterBottom sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
                                <InfoOutlined sx={{ mr: 1, color: 'primary.main' }} /> Observaciones
                            </Typography>
                            <Typography variant="body2" color="textSecondary" sx={{ whiteSpace: 'pre-wrap' }}>{order.observations}</Typography>
                        </Paper>
                    )}
                </Grid2>
            </Grid2>
