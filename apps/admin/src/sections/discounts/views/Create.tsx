...
const CreateDiscount: React.FC = () => {
  ...
  const productVariantOptions = useMemo((): ProductOrVariantOption[] => {
    ...
    allProducts.forEach((product) => {
      if (!product._id || !product.variants || product.variants.length === 0)
        return; // Skip products without ID or variants
      ...
      if (validPricesFound) {
        const formatRange = (
          ...
          if (min === Infinity || max === -Infinity)
            return `${prefix}: USD N/A`; // Handle case where only one price type had valid numbers
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      ); // Ensure products have variants for selection
    } catch (err: any) {
  ...
  const handleRemoveOverride = (tempIdToRemove: string) => {
    ...
    if (
      validationErrors?.priceValidation ||
      validationErrors?.entityOverrides
    ) {
      setValidationErrors((prevErrors) => {
        if (!prevErrors) return null;
        const updated = { ...prevErrors };
  ...
  const validateForm = (): boolean => {
      ...
      for (const [productId, variantId] of formData.applicableProducts) {
        ...
        if (
          ...
          invalidVariants.push(
            `${productName} - ${variant.name || "Variante sin nombre"}`,
          );
        }
      }
    ...
    if (Object.keys(errors).length > 0 && !errors.summary) {
      // Ensure summary exists if errors do
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload: Omit<Discount, "_id" | "applicableArts"> = {
      // Adjust Omit<>
      name: formData.name,
                            ...
                            <Grid2 size={{ xs: 12, sm: 6, md: 3 }}>
                              <TextField
                                ...
                                InputProps={{
                                  ...
                                  startAdornment:
                                    override.adjustmentMethod === "absolute" ? (
