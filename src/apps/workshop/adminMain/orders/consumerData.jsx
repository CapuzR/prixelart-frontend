...
export default function ConsumerData(props) {
  ...
  useEffect(() => {
    ...
    const updated = consumers?.filter((consumer) =>
      consumer.firstname?.toLowerCase().includes(props?.basicData?.name?.toLowerCase())
    );
    updated.map((p) => {
    ...
    const updatedv3 = prixers?.filter((prixer) =>
      prixer?.firstName?.toLowerCase().includes(props?.basicData?.name?.toLowerCase())
    );
  ...
  const handleInputChange = (event, value, reason) => {
    if (reason === 'clear') {
      ...
      let prixer = prixers.find(
        (prixer) => prixer?.firstName === valuev2[0] && prixer?.lastName === valuev2[1]?.trim()
      );
      let selected = consumers.find(
        (consumer) => consumer.firstname === valuev2[0] && consumer.lastname === valuev2[1]?.trim()
      );
      if (selected) {
      ...
      if (prixer) {
        ...
        prixer = prixers.find((prixer) => prixer?.username === selected.username);
        props.setSelectedPrixer(prixer);
      }
    }
  };
    ...
    <>
      <Grid container spacing={2}>
        ...
        <Grid container>
          ...
          <Grid item lg={4} md={4} sm={4} xs={12} className={classes.gridInput}>
            <FormControl style={{ minWidth: '100%', marginTop: 15 }} variant="outlined">
