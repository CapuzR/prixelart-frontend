...
export default function ReadMovements(props) {
  ...
  const getUsername = (fullname) => {
    ...
    if (name && name?.length === 2) {
      selected = orgs.find((p) => p?.firstName === name[0] && p?.lastName === name[1]);
      if (selected === undefined) {
        selected = prixers?.find((p) => p?.firstName === name[0] && p?.lastName === name[1]);
      }
                    ...
                    <TableRow>
                      ...
                      <TableCell align="center">
                        <FormControl className={classes.formControl}>
                          <InputLabel id="demo-simple-select-label">Destinatario</InputLabel>
                          <Select
                        ...
                        <TableRow key={row._id}>
                          ...
                          <TableCell align="center">
                            {row.description?.includes('#') ? (
                              <div>
                                <Typography>{row.description?.split('#')?.[0]}</Typography>
                                <Button
                                  onClick={() => {
