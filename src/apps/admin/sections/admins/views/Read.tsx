...
const headCells: readonly HeadCell[] = [
  { id: 'username', numeric: false, label: 'Nombre de Usuario', sortable: true },
  ...
  { id: 'actions', numeric: false, label: 'Acciones', sortable: false, minWidth: 120 },
];
...
function descendingComparator<T>(a: T, b: T, orderBy: keyof T) {
  ...
  if (valA && typeof valA === 'object' && typeof (valA as any).toHexString === 'function') {
    valA = (valA as any).toHexString();
  }
  if (valB && typeof valB === 'object' && typeof (valB as any).toHexString === 'function') {
...
export default function ReadAdmins() {
  ...
  const processedAdmins = useMemo(() => {
    ...
    if (filterIsSeller !== '') {
      ...
      filteredData = filteredData.filter(admin => admin.isSeller === sellerBool);
    }
    ...
    return { paginatedAdmins: paginatedData, filteredRowCount: filteredRowCount };

  }, [originalAdmins, searchTerm, filterArea, filterIsSeller, order, orderBy, page, rowsPerPage]);
  ...
  const handleCreate = () => { navigate('/admin/admins/create'); };
  const handleUpdate = (username: string) => { navigate(`/admin/admins/update/${username}`); };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showSnackBar(`Admin '${deletionTarget.displayName}' eliminado exitosamente.`);
      fetchAdmins();
    } catch (err: any) {
          ...
          >
            {headCell.sortable ? (
              <TableSortLabel
                ...
                onClick={() => headCell.id !== 'actions' && handleSortRequest(headCell.id as keyof Admin)}
  ...
  const renderContent = () => {
    ...
    if (error && originalAdmins.length === 0) {
      return (
        <Alert severity="error" sx={{ m: 2 }}>
          {error}
          <Button onClick={fetchAdmins} size="small" sx={{ ml: 1 }} disabled={isLoading}>Reintentar</Button>
        </Alert>
      );
    }
    ...
    return (
      <Paper sx={{ width: '100%', mb: 2, position: 'relative', overflow: 'hidden' }}>
        {isDeleting && (
        ...
        <TableContainer>
          <Table sx={{ minWidth: 750 }} aria-label="administrators table" size="medium">
            <EnhancedTableHead />
            <TableBody>
              {processedAdmins.paginatedAdmins.map((admin) => (
                <TableRow
                  key={admin._id ? admin._id.toString() : String(admin.username)}
                  hover
                  sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
                >
                  <TableCell component="th" scope="row">{admin.username}</TableCell>
                    ...
                    <Tooltip title="Eliminar Admin">
                      <span>
                        <IconButton
                          ...
                          disabled={isLoading || (isDeleting && deletionTarget?.username === admin.username)}
                          size="small"
                        >
  ...
  return (
    <>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        <Grid2 container spacing={2} alignItems="center" sx={{ '& .MuiGrid-item': { pt: { xs: 1, sm: '16px' }, pb: { xs: 1, sm: 0 } } }}> {/* Responsive spacing */}
          {/* Search Input */}
          <Grid2 size={{ xs: 12, sm: 6, md: 4 }}>
            <TextField
              fullWidth label="Buscar por nombre, e‑mail, área, vendedor…" variant="outlined" size="small"
          ...
          <Grid2 size={{ xs: 6, sm: 3, md: 2 }}>
            <FormControl fullWidth size="small" variant="outlined">
              ...
              <Select
                ...
                value={filterArea} onChange={handleFilterAreaChange} label="Área"
                disabled={isLoading || isDeleting || availableAreas.length === 0}
              >
          ...
          <Grid2 size={{ xs: 6, sm: 3, md: 2 }}>
            <FormControl fullWidth size="small" variant="outlined">
              ...
              <Select
                ...
                value={filterIsSeller} onChange={handleFilterIsSellerChange} label="Es Vendedor"
                disabled={isLoading || isDeleting}
              >
                <MenuItem value=""><em>Todos</em></MenuItem>
          ...
          <Grid2 size={{ xs: 12, sm: 6, md: "auto" }} >
            <Button
              ...
              disabled={isLoading || isDeleting} fullWidth
      ...
      {/* Confirmation Dialog for Deletion */}
      {
        deletionTarget && (
          <ConfirmationDialog
            ...
            message={
              <>
                ¿Estás seguro de que deseas eliminar permanentemente al administrador{' '}
                <strong>{deletionTarget.displayName}</strong>? Esta acción no se puede deshacer.
              </>
            }
