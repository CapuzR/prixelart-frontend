...
const AllServicesDisplay: React.FC = () => {
  ...
  const [prixerDetailsMap, setPrixerDetailsMap] = useState<Map<string, { username: string; avatar?: string }>>(new Map());
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      ...
      const newPrixerDetailsMap = new Map<string, { username: string; avatar?: string }>();
      fetchedUsers.forEach(user => {
        if (user?.prixer?._id) {
          newPrixerDetailsMap.set(
            user.prixer._id.toString(),
            {
  ...
  if (isLoading) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" sx={{ minHeight: '70vh' }}>
        ...
        <Typography variant="h6" sx={{ ml: 2 }}>Cargando servicios...</Typography>
      </Box>
    );
  }
    ...
    return (
      <Container maxWidth="md" sx={{ py: 4, textAlign: 'center' }}>
        <Alert
          ...
          action={<Button color="inherit" size="small" onClick={loadAllData}> RETRY </Button>}
  ...
  const servicesToDisplay = rawServices.map(service => {
    ...
    if (!prixerDetails) {
      console.warn(`Skipping service "${service.title}" due to missing prixer details for ID: ${prixerIdString}`);
      return null;
    }

    return (
      <Grid2 size={{ xs: 12, sm: 6, md: 4, lg: 3 }}
        key={service._id ? service._id.toString() : `${service.title}-${prixerIdString}`}
      >
        <ServiceCardMui
          service={service}
    ...
    return (
      <Container maxWidth="sm" sx={{ py: 8, textAlign: 'center' }}>
        <Paper elevation={0} sx={{ p: 4, backgroundColor: 'transparent' }}>
          ...
          <Typography variant="h5" gutterBottom> No hay servicios en este momento </Typography>
          <Typography color="text.secondary">
            Ha ocurrido un error al cargar los servicios. Por favor, inténtalo de nuevo más tarde.
          </Typography>
        </Paper>
      </Container>
    );
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Typography variant="h3" component="h1" gutterBottom align="center" sx={{ mb: 1, fontWeight: 600 }}>
        Servicios
      </Typography>
      <Typography variant="h6" component="p" color="text.secondary" align="center" sx={{ mb: 6 }}>
        Explora nuestros servicios disponibles y descubre lo que tenemos para ofrecerte.
      </Typography>
