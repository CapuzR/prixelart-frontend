...
const ReadPaymentMethods: React.FC = () => {
    ...
    async (showLoading = true) => {
      ...
      try {
        const fetchedMethods =
          (await readAllPaymentMethods()) as PaymentMethod[];
        if (fetchedMethods.some((m) => !m._id))
          console.error("Some methods missing '_id'.");
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deletePaymentMethod(methodToDelete._id.toString());
  ...
  const handleUpdate = (methodId: string) => {
    ...
    navigate(`/admin/payment-method/update/${methodId}`);
  }; // Adjust route
  const handleCreate = () => {
    /* ... */ navigate("/admin/payment-method/create");
  }; // Adjust route

  // --- Table Head Configuration ---
  interface HeadCell {
    id: PaymentMethodSortableColumns;
    label: string;
    numeric: boolean;
  ...
  return (
    <>
      ...
      {error && !isLoading && paymentMethods.length === 0 && (
              ...
              <TableBody>
                {processedMethods.map((method) => (
                  ...
                  >
                    ...
                    {/* Created On Date */}
                    <TableCell>
                      {new Date(method.createdOn).toLocaleDateString()}
                    </TableCell>
