...
export default function PrixerGrid(): JSX.Element {
  ...
  useEffect(() => {
    const base_url =
      import.meta.env.VITE_BACKEND_URL + "/prixer/read-all-full-v2"
  ...
  return (
    <div className={classes.root}>
      ...
      <Grid2
        container
        spacing={2}
        style={{ padding: 10, width: "100%", justifyContent: "center" }}
      >
        {tiles &&
          tiles
            ...
            .map(
              (tile) =>
                tile.status && (
                  <Grid2 key={tile._id} size={{ lg: 3, md: 5, sm: 6, xs: 12 }}>
                    <Paper elevation={5} className={classes.card}>
                      <CardMedia
                        image={tile.avatar}
                        className={classes.cardMedia}
                        title={
                          tile.title || `${tile.firstName} ${tile.lastName}`
                        }
                      />
