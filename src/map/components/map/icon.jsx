...
export const Icon = ({
  icon,
  rawMapRef,
  calculateIconRelation,
  setSelectedIcon,
  setOpenSelected,
}) => {
  let iconRef = React.useRef(React.createRef());
  let [iconRelatives, setIconRelatives] = React.useState({
    size: { x: 0, y: 0 },
    position: { x: 0, y: 0 },
  });
  ...
  const handleIconRelation = () => {
    const originalMapSize = {
      x: rawMapRef.current?.naturalWidth,
      y: rawMapRef.current?.naturalHeight,
    };
    const originalIconSize = {
      x: iconRef.current?.naturalWidth,
      y: iconRef.current?.naturalHeight,
    };
    // console.log(icon.originalCoordinates?.y);
    const originalIconPosition = {
      x: icon.originalCoordinates?.x,
      y: icon.originalCoordinates?.y,
    };
    const screenMapSize = {
      x: rawMapRef.current?.width,
  ...
  React.useEffect(() => {
    handleIconRelation();
  }, [icon, rawMapRef, calculateIconRelation, setSelectedIcon]);
