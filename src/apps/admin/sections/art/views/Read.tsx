...
import {
    ...
    Tooltip, Stack, Chip, Avatar, TextField, InputAdornment,
    Select, MenuItem, FormControl, InputLabel, Toolbar, TablePagination,
...
const ReadArts: React.FC = () => {
    ...
    // --- Fetch Data ---
    const loadArtsAndOwners = useCallback(async (showLoading = true) => {
        if (showLoading) setIsLoading(true);
        setError(null); setOwnerInfoMap({}); setAvailableCategories([]); setAvailableStatuses([]);
        ...
        try {
            ...
            const ownerIds = Array.from(new Set(fetchedArts.map(art => art.userId?.toString()).filter((id): id is string => !!id)));
            ...
            const message = err.message || "Error al cargar datos."; setError(message); showSnackBar(message);
        } finally { if (showLoading) setIsLoading(false); }
    }, [showSnackBar]);

    useEffect(() => { loadArtsAndOwners(); }, [loadArtsAndOwners]);
    ...
    const filteredArts = useMemo(() => {
        return arts.filter(art => {
            const ownerName = ownerInfoMap[art.userId?.toString()]?.name?.toLowerCase() || art.prixerUsername?.toLowerCase() || '';
            const searchLower = debouncedSearchTerm.toLowerCase();

            // Filter checks
            const statusMatch = filterStatus === 'all' || art.status?.toLowerCase() === filterStatus.toLowerCase();
            const visibilityMatch = filterVisibility === 'all' || (art.visible ? 'yes' : 'no') === filterVisibility;
            const categoryMatch = filterCategory === 'all' || art.category === filterCategory;

            // --- NEW: Tag Filter Check (AND logic - art must have ALL selected tags) ---
            const tagMatch = filterTags.size === 0 || (
                Array.from(filterTags).every(filterTag =>
                    art.tags?.some(artTag => artTag.toLowerCase() === filterTag.toLowerCase()) // Case-insensitive tag check
                )
            );
            // --- End NEW ---
            ...
            // Combine all filters
            return statusMatch && visibilityMatch && categoryMatch && tagMatch && searchMatch; // Added tagMatch
        });
        // Add filterTags to dependency array
    }, [arts, debouncedSearchTerm, filterStatus, filterVisibility, filterCategory, filterTags, ownerInfoMap]);
    ...
    const sortedArts = useMemo(() => {
        ...
        const comparator = (a: Art, b: Art): number => {
            ...
            if (orderBy === 'ownerName') {
                const nameA = ownerInfoMap[a.userId?.toString()]?.name || a.prixerUsername || '';
                const nameB = ownerInfoMap[b.userId?.toString()]?.name || b.prixerUsername || '';
                return order === 'desc'
    ...
    const handleOpenDeleteDialog = (art: Art) => { setArtToDelete(art); setDialogOpen(true); };
    const handleCloseDialog = () => { if (isDeleting) return; setDialogOpen(false); setArtToDelete(null); };
    ...
    const handleUpdate = (id: string | undefined) => { if (id) navigate(`/admin/art/update/${id}`); };
    const handleCreate = () => { navigate('/admin/art/create'); };
    ...
    // Filter Handlers
    const handleFilterChange = (event: ChangeEvent<{ name?: string; value: unknown }>, filterSetter: React.Dispatch<React.SetStateAction<string>>) => {
    ...
    const renderStatusChip = (status?: string) => {
        ...
        if (lowerStatus === 'approved' || lowerStatus === 'aprobada') { color = "success"; icon = <CheckCircleIcon />; }
        else if (lowerStatus === 'pending' || lowerStatus === 'pendiente') { color = "warning"; icon = <PendingIcon />; }
        else if (lowerStatus === 'rejected' || lowerStatus === 'rechazada') { color = "error"; icon = <CancelIcon />; }

        return <Chip label={status} color={color} size="small" icon={icon || undefined} variant="outlined" />;
    };
    ...
    return (
        <>
            ...
            <Paper sx={{ mb: 2, p: 2 }}>
                <Toolbar disableGutters sx={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'stretch' }}>
                    {/* Row 1: Search, Main Filters, Actions */}
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, width: '100%' }}>
                        {/* Search */}
                        <TextField
                            sx={{ flexBasis: { xs: '100%', sm: 'auto' }, flexGrow: 1, minWidth: '200px' }}
                            value={searchTerm}
                        ...
                        <FormControl size="small" sx={{ flexBasis: { xs: '45%', sm: 'auto' }, minWidth: 120 }}>
                            ...
                            <Select labelId="visibility-filter-label" label="Visibilidad" name="visibility" value={filterVisibility} onChange={(e) => handleFilterChange(e as any, setFilterVisibility)}>
                        ...
                        <FormControl size="small" sx={{ flexBasis: { xs: '45%', sm: 'auto' }, minWidth: 120 }}>
                            ...
                            <Select labelId="category-filter-label" label="Categoría" name="category" value={filterCategory} onChange={(e) => handleFilterChange(e as any, setFilterCategory)}>
                                <MenuItem value="all">Todas</MenuItem> {availableCategories.map(cat => <MenuItem key={cat} value={cat}>{cat}</MenuItem>)}
                            </Select>
                        </FormControl>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, flexBasis: { xs: '100%', sm: 'auto' }, justifyContent: 'flex-end', ml: 'auto' }}>
                            ...
                            <Button variant="contained" startIcon={<AddIcon />} onClick={handleCreate} disabled={isLoading || isDeleting}>Crear Arte</Button>
                        </Box>
                    </Box>

                    {/* --- NEW: Row 2: Active Tag Filters --- */}
                    {filterTags.size > 0 && (
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, alignItems: 'center', pt: 1, borderTop: 1, borderColor: 'divider' }}>
                            <Typography variant="body2" sx={{ mr: 1, fontWeight: 'medium' }}>Filtros por Tags:</Typography>
            ...
            {error && !isLoading && filteredArts.length === 0 && (<Alert severity="error" sx={{ m: 2 }}>{error}<Button onClick={() => loadArtsAndOwners()} size="small" sx={{ ml: 1 }}>Reintentar</Button></Alert>)}
            {error && !isLoading && filteredArts.length > 0 && (<Alert severity="warning" sx={{ mb: 2 }}>No se pudo actualizar la lista: {error}</Alert>)}
            {!isLoading && filteredArts.length === 0 && !error && (<Alert severity="info" sx={{ m: 2 }}>No se encontraron obras que coincidan con los criterios.</Alert>)}
                    ...
                    <Table sx={{ minWidth: 900 }} aria-label="art table">
                        <TableHead sx={{ backgroundColor: (theme) => theme.palette.action.hover }}>
                            <TableRow>
                                ...
                                <TableCell sx={{ fontWeight: 'bold', width: '25%' }}><TableSortLabel active={orderBy === 'title'} direction={orderBy === 'title' ? order : 'asc'} onClick={() => handleRequestSort('title')}>Título</TableSortLabel></TableCell>
                                <TableCell sx={{ fontWeight: 'bold', width: '15%' }}><TableSortLabel active={orderBy === 'ownerName'} direction={orderBy === 'ownerName' ? order : 'asc'} onClick={() => handleRequestSort('ownerName')}>Propietario</TableSortLabel></TableCell>
                                <TableCell sx={{ fontWeight: 'bold', width: '12%' }}><TableSortLabel active={orderBy === 'category'} direction={orderBy === 'category' ? order : 'asc'} onClick={() => handleRequestSort('category')}>Categoría</TableSortLabel></TableCell>
                                <TableCell sx={{ fontWeight: 'bold', width: '15%' }}>Tags</TableCell> {/* Tags column not sortable easily client-side */}
                                <TableCell sx={{ fontWeight: 'bold', width: '10%' }}><TableSortLabel active={orderBy === 'status'} direction={orderBy === 'status' ? order : 'asc'} onClick={() => handleRequestSort('status')}>Estado</TableSortLabel></TableCell>
                                <TableCell sx={{ fontWeight: 'bold', width: '5%' }}><TableSortLabel active={orderBy === 'visible'} direction={orderBy === 'visible' ? order : 'asc'} onClick={() => handleRequestSort('visible')}>Visible</TableSortLabel></TableCell>
                        ...
                        <TableBody>
                            {paginatedArts.map((art) => {
                                /* ownerInfo lookup */
                                const ownerInfo = ownerInfoMap[art.userId?.toString()] || { name: art.prixerUsername || `ID: ${art.userId}`, avatar: undefined };
                                return (
                                    <TableRow hover key={art._id?.toString() || art.artId} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                        {/* Other Cells */}
                                        <TableCell><Avatar variant="rounded" src={art.squareThumbUrl || art.smallThumbUrl} alt={art.title} sx={{ width: 56, height: 56 }} /></TableCell>
                                        <TableCell component="th" scope="row"><Tooltip title={art.title} placement="top-start"><Typography variant="body2" noWrap sx={{ maxWidth: 250 }}>{art.title}</Typography></Tooltip><Typography variant="caption" display="block" color="textSecondary">ID: {art.artId}</Typography></TableCell>
                                        <TableCell><Tooltip title={ownerInfo.name} placement="top-start"><Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><Avatar src={ownerInfo.avatar} sx={{ width: 24, height: 24 }} /><Typography variant="body2" noWrap sx={{ maxWidth: 150 }}>{ownerInfo.name}</Typography></Box></Tooltip></TableCell>
                                        ...
                                        <TableCell>
                                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, maxWidth: 200 }}>
                                                ...
                                                {(art.tags || []).slice(0, 5).map(tag => {
                                                    ...
                                                    return (
                                                        <Tooltip key={tag} title={isActive ? `Quitar filtro "${tag}"` : `Filtrar por "${tag}"`}>
                                                            <Chip
                                                                label={tag}
                                                                size="small"
                                        ...
                                        <TableCell align='center'><Tooltip title={art.visible ? 'Visible' : 'Oculto'}>{art.visible ? <VisibilityIcon color="success" /> : <VisibilityOffIcon color="action" />}</Tooltip></TableCell>
                                        <TableCell align="right">{/* Action Buttons */}<Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.5 }}><Tooltip title="Editar Arte"><IconButton aria-label="edit" color="primary" onClick={() => handleUpdate(art._id?.toString())} disabled={!art._id || isDeleting} size="small"><EditIcon fontSize="small" /></IconButton></Tooltip><Tooltip title="Eliminar Arte"><IconButton aria-label="delete" color="error" onClick={() => handleOpenDeleteDialog(art)} disabled={!art._id || (isDeleting && artToDelete?._id === art._id)} size="small"><DeleteIcon fontSize="small" /></IconButton></Tooltip></Box></TableCell>
                                    </TableRow>
                                );
                            })}
                        </TableBody>
            ...
            <ConfirmationDialog
                ...
                message={<>¿Estás seguro de eliminar la obra <strong>{artToDelete?.title || ''}</strong>?</>}
