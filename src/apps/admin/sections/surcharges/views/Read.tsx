...
import {
    ...
    Tooltip, Stack, Chip, TextField, InputAdornment, Select, MenuItem,
...
function getNestedValue(obj: any, path: string): any {
    ...
    return value === null || typeof value === 'undefined' ? (typeof value === 'number' ? -Infinity : '') : value;
}
...
const formatApplicability = (surcharge: Surcharge): string => {
    ...
    else if (surcharge.applicableProducts && surcharge.applicableProducts.length > 0) parts.push(`${surcharge.applicableProducts.length} Prod.`);

    if (surcharge.appliestoAllArts) parts.push("Todas Artes");
    else if (surcharge.applicableArts && surcharge.applicableArts.length > 0) parts.push(`${surcharge.applicableArts.length} Arte(s)`);

    // Check if *only* entity-specific rules apply (overrides or recipients)
    const onlyEntities = parts.length === 0 &&
    ...
    if (parts.includes("Todos Productos") && parts.includes("Todas Artes")) return "Todo";
...
const formatDateRange = (range?: { start: Date | string; end: Date | string }): string => {
...
const ReadSurcharges: React.FC = () => {
    ...
    const [surchargeToDelete, setSurchargeToDelete] = useState<Surcharge | null>(null);
    ...
    useEffect(() => {
        const handler = setTimeout(() => { setDebouncedSearchTerm(searchTerm); setPage(0); }, 300); // Reset page on search
        return () => { clearTimeout(handler); };
    }, [searchTerm]);

    // --- Fetch Data ---
    const loadSurcharges = useCallback(async (showLoading = true) => {
        ...
        try {
            ...
            if (fetchedSurcharges.some(s => !s._id)) {
                console.warn("Some fetched surcharges are missing '_id'. Ensure API returns it.");
            }
    ...
    // Initial data load
    useEffect(() => { loadSurcharges(); }, [loadSurcharges]);
    ...
    const filteredSurcharges = useMemo(() => {
        ...
        return surcharges.filter(surcharge => {
            ...
            const statusMatch = filterStatus === 'all' || (surcharge.active ? 'active' : 'inactive') === filterStatus;

            // Adjustment Method Filter
            const methodMatch = filterMethod === 'all' || surcharge.adjustmentMethod === filterMethod;
            ...
            if (filterCurrentlyActive !== 'all') {
                const hasDateRange = !!surcharge.dateRange?.start && !!surcharge.dateRange?.end;
                if (!hasDateRange) {
                    ...
                    try {
                        ...
                        console.error("Error parsing date for filter:", surcharge.dateRange, e);
    ...
    }, [surcharges, debouncedSearchTerm, filterStatus, filterMethod, filterCurrentlyActive]);
    ...
    const handleFilterChange = (event: ChangeEvent<{ name?: string; value: unknown }>, filterSetter: React.Dispatch<React.SetStateAction<any>>) => {
    ...
    const handleOpenDeleteDialog = (surcharge: Surcharge) => {
        if (!surcharge._id) { showSnackBar("Error: Falta ID del recargo."); return; }
    ...
    const handleConfirmDelete = async () => {
        if (!surchargeToDelete?._id) { showSnackBar("Error: Recargo no seleccionado para eliminar."); handleCloseDialog(); return; }
        setIsDeleting(true);
        try {
    ...
    const handleUpdate = (surchargeId: ObjectId | string | undefined) => {
        if (!surchargeId) { showSnackBar("Error: Falta ID para editar."); return; }
        ...
        <>
            ...
            <Paper sx={{ mb: 2, p: 2 }}>
                <Toolbar disableGutters sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                    ...
                    <TextField
                        ...
                        variant="outlined" size="small" placeholder="Buscar Nombre, Descripción..."
                    ...
                    <FormControl size="small" sx={{ flexBasis: { xs: '45%', sm: 'auto' }, minWidth: 120 }}>
                        ...
                        <Select
                            labelId="status-filter-label" label="Estado" name="status"
                            value={filterStatus}
                    ...
                    <FormControl size="small" sx={{ flexBasis: { xs: '45%', sm: 'auto' }, minWidth: 120 }}>
                        ...
                        <Select
                            labelId="method-filter-label" label="Método" name="method"
                            value={filterMethod}
                     ...
                     <FormControl size="small" sx={{ flexBasis: { xs: '45%', sm: 'auto' }, minWidth: 140 }}>
                        ...
                        <Select
                            labelId="active-now-filter-label" label="Activo Ahora" name="activeNow"
                            value={filterCurrentlyActive}
                            onChange={(e) => handleFilterChange(e as any, setFilterCurrentlyActive)}
                        >
                            <MenuItem value="all">Todos</MenuItem>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, flexBasis: { xs: '100%', sm: 'auto' }, justifyContent: 'flex-end', ml: 'auto' }}>
                        <Tooltip title="Refrescar Lista">
                            ...
                            <span><IconButton onClick={handleRefresh} disabled={isLoading || isDeleting}><RefreshIcon /></IconButton></span>
                        </Tooltip>
                        <Button variant="contained" startIcon={<AddIcon />} onClick={handleCreate} disabled={isLoading || isDeleting}>
                            Crear Recargo
                        </Button>
                    </Box>
                </Toolbar>
            ...
            {error && !isLoading && filteredSurcharges.length === 0 && (
                <Alert severity="error" sx={{ m: 2 }}>
                    {error}
                    <Button onClick={() => loadSurcharges()} size="small" sx={{ ml: 1 }}>Reintentar</Button>
                </Alert>
            )}
            {/* Error during refresh when data *is* present */}
            {error && !isLoading && filteredSurcharges.length > 0 && (
                <Alert severity="warning" sx={{ mb: 2 }}>No se pudo actualizar la lista: {error}</Alert>
            )}

            {/* No Results State */}
            {!isLoading && !error && filteredSurcharges.length === 0 && (
                <Alert severity="info" sx={{ m: 2 }}>
                    No se encontraron recargos que coincidan con los criterios de búsqueda y filtros.
                </Alert>
            )}
                    ...
                    <Table sx={{ minWidth: 800 }} aria-label="surcharges table">
                        ...
                        <TableHead sx={{ backgroundColor: (theme) => theme.palette.action.hover }}>
                            <TableRow>
                                ...
                                <TableCell sx={{ width: '20%', fontWeight: 'bold' }}>Aplica A</TableCell> {/* Not sortable by default */}
                        ...
                        <TableBody>
                            {paginatedSurcharges.map((surcharge) => (
                                <TableRow hover key={surcharge._id?.toString()} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                    <TableCell component="th" scope="row">
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                            {surcharge.name}
                                            {/* Overrides Indicator */}
                                            {surcharge.entityOverrides && surcharge.entityOverrides.length > 0 && (
                                                <Tooltip title={`Tiene ${surcharge.entityOverrides.length} excepción(es) de entidad`}>
                                                    <InfoOutlinedIcon color="action" sx={{ fontSize: '1rem', verticalAlign: 'middle' }} />
                                                </Tooltip>
                                            )}
                                            {/* Recipients Indicator */}
                                            {surcharge.recipients && surcharge.recipients.length > 0 && (
                                                <Tooltip title={`Tiene ${surcharge.recipients.length} receptor(es) específico(s)`}>
                                                     {/* Use a different icon for recipients */}
                                                    <PeopleAltOutlinedIcon color="action" sx={{ fontSize: '1rem', verticalAlign: 'middle', ml: 0.2 }} />
                                                </Tooltip>
                                            )}
                                        </Box>
                                        <Tooltip title={surcharge.description || "Sin descripción"} placement="bottom-start">
                                            <Typography variant="caption" display="block" color="text.secondary" noWrap sx={{ maxWidth: 300 }}>
                                                {surcharge.description || ""} {/* Show empty string if no description */}
                                            </Typography>
                                        </Tooltip>
                                    </TableCell>
                                    <TableCell>{formatAdjustmentValue(surcharge.defaultValue, surcharge.adjustmentMethod)}</TableCell>
                                    <TableCell>{formatApplicability(surcharge)}</TableCell>
                                    <TableCell>{formatDateRange(surcharge.dateRange)}</TableCell>
                                    <TableCell>
                                        <Chip
                                            icon={surcharge.active ? <CheckCircleIcon /> : <CancelIcon />}
                                        ...
                                        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.5 }}>
                                            ...
                                            <Tooltip title="Eliminar Recargo">
                                                ...
                                                <IconButton
                                                    ...
                                                    disabled={!surcharge._id || (isDeleting && surchargeToDelete?._id === surcharge._id)}
                                                >
            ...
            <ConfirmationDialog
                ...
                message={<>¿Estás seguro de que deseas eliminar el recargo <strong>{surchargeToDelete?.name || ''}</strong>?</>}
