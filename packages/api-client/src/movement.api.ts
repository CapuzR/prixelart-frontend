...
export const fetchMovementById = async (
  movementId: string,
): Promise<Movement> => {
...
export const getMovements = async (
  ...
  try {
    const response = await axios.get<GetMovementsResponse>(url, {
      withCredentials: true,
    });

    if (!response.data.success || !response.data.result) {
      console.error(
        "Backend reported failure fetching movements:",
        response.data.message,
      );
    ...
    if (axios.isAxiosError(e)) {
      throw new Error(
        `Network or server error: ${e.response?.data?.message || e.message}`,
      );
    }
    throw e;
  }
...
export const updateMovement = async (
  ...
  try {
    ...
    if (!response.data.success) {
      console.error(
        `Backend reported failure updating movement`,
        response.data.message,
      );
    ...
    if (!response.data.result) {
      console.error(
        `Backend reported success but no movement data returned after update.`,
      );
