...
export const RolePermissionsDetails: React.FC<{ role: PermissionsV2 }> = ({
  role,
}) => {
...
const ReadRoles: React.FC = () => {
  ...
  const [sortConfig, setSortConfig] = useState<{
    key: keyof PermissionsV2 | null;
    direction: "asc" | "desc";
  }>({ key: "area", direction: "asc" });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setRoles((prevRoles) =>
        prevRoles.filter((role) => role._id?.toString() !== roleIdString),
      );
      handleCloseDialog();
    } catch (err: any) {
  ...
  const processedRoles = useMemo(() => {
    ...
    if (sortConfig.key === "area") {
      // Only sorting by area implemented
      processed.sort((a, b) => {
    ...
    if (error && roles.length === 0) {
      return (
        <Alert severity="error" sx={{ m: 2 }}>
          {error}
          <Button onClick={loadRoles} size="small" sx={{ ml: 1 }}>
            Reintentar
          </Button>
        </Alert>
      );
    ...
    if (!isLoading && roles.length === 0 && !error) {
      return (
        <Alert severity="info" sx={{ m: 2 }}>
          No se encontraron roles. Puede crear uno nuevo usando el botón de
          arriba.
        </Alert>
      );
    }
          ...
          <Table sx={{ minWidth: 650 }} aria-label="roles expandable table">
            ...
            >
              <TableRow>
                ...
                <TableCell
                  sortDirection={
                    sortConfig.key === "area" ? sortConfig.direction : false
                  }
                  sx={{ fontWeight: "bold" }}
                >
                  <TableSortLabel
                          ...
                          <Tooltip title="Eliminar Rol">
                            <span>
                              <IconButton
                                ...
                                disabled={
                                  isDeleting &&
                                  roleToDelete?._id?.toString() === roleIdString
                                }
                                size="small"
                              >
                          ...
                          <Box
                            sx={{
                              ...
                              backgroundColor: (theme) =>
                                theme.palette.background.default, // Slight background difference
                            }}
                          >
                            <Typography
                              variant="h6"
                ...
                (searchTerm || selectedFilters.length > 0) && (
                  <TableRow>
                    <TableCell colSpan={3} align="center">
                      No se encontraron roles que coincidan con los criterios de
                      búsqueda o filtrado.
                    </TableCell>
                  </TableRow>
                )}
            ...
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                renderValue={(selected) =>
                  selected.length === 0 ? <em>Todos</em> : selected.join(", ")
                }
                MenuProps={{
                  // Prevent menu from going too wide
                  PaperProps: {
                    style: {
                      maxHeight: 300,
                    },
              ...
              >
                ...
                {permissionGroups.map((group) => (
                  <MenuItem key={group.title} value={group.title}>
                    <Checkbox
                      checked={selectedFilters.includes(group.title)}
                      size="small"
                    />
      ...
      <ConfirmationDialog
        ...
        message={
          <>
            ...
            <strong>{roleToDelete?.area || ""}</strong>? Esta acción no se puede
            deshacer.
          </>
        }
