...
export default function ArtDetail(props) {
  ...
  const [fullArt, setFullArt] = useState(props.fullArt?.artId || globalParams.get('/art'));
          ...
          {tiles ? (
            tilesv2.map((tile) =>
              artDataState !== tile.artId ? (
                <div id={tile.artId} key={tile.artId}>
                  {tile.visible === true ? (
                    <>
                            ...
                            >
                              <Grid item>
                                <Button
                                  size="small"
                                  variant="outlined"
                                  onClick={(e) => navigateToPrixer(e, tile.prixerUsername)}
                                >
                                  <Typography
                                    gutterBottom
                                    variant="h7"
                                    ...
                                    <IconButton
                                      onClick={(e) => {
                                        if (
                                          ...
                                          setSequence(tile?.certificate?.sequence);
                      ...
                      <Dialog open={hiddenArt === tile.artId} onClose={handleCloseVisible}>
                ...
                <Card id={tile.artId} key={tile.artId} style={{ marginTop: 35 }}>
                  <Img
                    placeholder="/imgLoading.svg"
                                ...
                                value.map((option, index) => (
                                  <Chip
                                    ...
                                    onDelete={() => {
                                      ...
                                      newTiles.find((item) => item.artId === tile.artId).tags =
                                        tile.tags.filter((tag) => tag !== option);
                                      setTiles(newTiles);
                                    }}
                                    variant="outlined"
                                    label={option}
                                ...
                                <TextField
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === 'Enter' && e.target.value) {
                                      const newTiles = [...tiles];
                                      newTiles
                                        .find((item) => item.artId === tile.artId)
        ...
        >
          <div className={classes.paper2}>
            ...
            <div style={{ justifyContent: 'center', display: 'flex' }}>
              <Button
                onClick={(e) => {
                  handleSubmit(e, JSON.parse(localStorage.getItem('token')).username);
                }}
                type="submit"
                variant="contained"
                color="primary"
