...
const CreateTestimonial: React.FC = () => {
    ...
    const loadPositionOptions = useCallback(async () => {
        ...
        try {
            ...
            const existingTestimonials = (await readAllTestimonial() as Testimonial[])
            ...
            if (existingTestimonials.length > 0) {
                ...
                const lastPosition = Math.max(...existingTestimonials.map(t => t.position), 0) + 1;
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.name.trim()) {
            ...
            setErrorSubmit(message); showSnackBar(message); return false;
        }
        ...
        if (!formData.avatar.trim()) { // Basic check, could validate URL format
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response) { // Adjust check based on API
                ...
                throw new Error("La creación del testimonio no devolvió una respuesta esperada.");
            }
        } catch (err: any) {
                    ...
                    <form onSubmit={handleSubmit}>
                        <Grid2 container spacing={3}>
                            ...
                            <Grid2 size={{ xs: 12, sm: 6 }}>
                                <TextField label="Nombre" name="name" value={formData.name} onChange={handleInputChange} required fullWidth disabled={isSubmitting} error={!!errorSubmit && !formData.name.trim()} />
                            </Grid2>
                            {/* Type */}
                            <Grid2 size={{ xs: 12, sm: 6 }}>
                                <TextField label="Tipo (Ej: Cliente, Partner)" name="type" value={formData.type} onChange={handleInputChange} required fullWidth disabled={isSubmitting} error={!!errorSubmit && !formData.type.trim()} />
                            </Grid2>
                            {/* Value (Testimonial Text) */}
                            <Grid2 size={{ xs: 12 }}>
                                <TextField label="Testimonio (Valor)" name="value" value={formData.value} onChange={handleInputChange} required fullWidth multiline rows={4} disabled={isSubmitting} error={!!errorSubmit && !formData.value.trim()} />
                            </Grid2>
                            {/* Avatar URL */}
                            <Grid2 size={{ xs: 12, sm: 6 }}>
                                <TextField label="URL del Avatar" name="avatar" type="url" value={formData.avatar} onChange={handleInputChange} required fullWidth disabled={isSubmitting} error={!!errorSubmit && !formData.avatar.trim()} />
                            </Grid2>
                            {/* Footer (Optional) */}
                            <Grid2 size={{ xs: 12, sm: 6 }}>
                                <TextField label="Pie de página (Opcional)" name="footer" value={formData.footer} onChange={handleInputChange} fullWidth disabled={isSubmitting} />
                            </Grid2>
                            ...
                            <Grid2 size={{ xs: 12, sm: 6 }} sx={{ display: 'flex', alignItems: 'center' }}>
                                <FormControlLabel
                                    control={<Checkbox checked={formData.status} onChange={handleInputChange} name="status" disabled={isSubmitting} />}
                            ...
                            <Grid2 size={{ xs: 12 }}>
                                <Stack direction="row" justifyContent="flex-end" spacing={2} sx={{ mt: 2 }}>
                                    ...
                                    <Button type="submit" variant="contained" color="primary" disabled={isSubmitting || isLoadingPositions} startIcon={isSubmitting ? <CircularProgress size={20} color="inherit" /> : null}>
