...
describe('with valid input values', () => {
  it('Inputting form login states works correctly', () => {
    ...
    const { getByTestId } = render(
      <div>
        <Select native={true} onChange={mockCallback} data-testid="select" defaultValue="Ambas">
    ...
    fireEvent.click(specialty.childNodes[0].childNodes[0], {
      target: { value: 'fotografía', textContent: 'fotografía' },
    });
    fireEvent.change(instagram, {
      target: { value: '@capuzr', textContent: '@capuzr' },
    });
    fireEvent.change(dateOfBirth, {
      target: { value: '12/05/1991', textContent: '12/05/1991' },
    });
    fireEvent.change(phone, {
      target: { value: '+584143201028', textContent: '+584143201028' },
    });
    fireEvent.change(country, {
      target: { value: 'Venezuela', textContent: 'Venezuela' },
    });
    fireEvent.change(city, {
      target: { value: 'Caracas', textContent: 'Caracas' },
    });
  ...
  it('calls api with data from form with all fields', async () => {
    ...
    await waitFor(async () => {
      fireEvent.click(specialty.childNodes[0].childNodes[0], {
        target: { value: 'Fotógrafía', textContent: 'Fotógrafía' },
      });
      fireEvent.change(instagram, {
        target: { value: '@capuzr', textContent: '@capuzr' },
      });
