...
const Flow = () => {
  ...
  useEffect(() => {
    const state = location.state as { autoAddToCart?: boolean } | null;
    if (isItemReady && item.price !== 'Error' && state?.autoAddToCart) {
      window.history.replaceState({}, document.title);

      handleCart(item as Item);

      window.history.replaceState(
        { ...window.history.state, autoAddToCart: false },
        document.title
      );
    }
  }, [isItemReady, item.price, location.state]);

  const handleArtSelect = (selectedArt: Art, shouldAddToCart: boolean = false) => {
    const excludedKeys = new Set(['producto', 'arte', 'lineId', 'itemId']);
    ...
    navigate(`${location.pathname}?${newQueryString}`, {
      state: { autoAddToCart: shouldAddToCart, timestamp: Date.now() },
    });
    showSnackBar('¡Arte seleccionado!');
  };
