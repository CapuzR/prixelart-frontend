...
export const ADJUSTMENT_METHOD_OPTIONS: { value: AdjustmentMethod; label: string }[] = [
...
export const getProductImageUrl = (product?: Product, variant?: Variant): string | undefined => {
    if (variant?.variantImage) return variant.variantImage;
    if (product?.thumbUrl && product.thumbUrl !== 'undefined') return product.thumbUrl; // Check for 'undefined' string just in case
...
export const calculateDiscountedPrice = (
    ...
    if (originalPriceStr === undefined || originalPriceStr === null || value === undefined || value === null) {
