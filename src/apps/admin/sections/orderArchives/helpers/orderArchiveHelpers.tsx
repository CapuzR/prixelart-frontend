...
export const formatDate = (dateInput?: { $date: Date | string | number } | string | Date | number): string => {
    ...
    if (typeof dateInput === 'object' && dateInput !== null && !(dateInput instanceof Date) && '$date' in dateInput) {
    ...
    try {
        ...
        if (isNaN(date.getTime())) { // Or use date-fns: if (!isValid(date))
...
export const formatCurrency = (value?: number | null): string => {
    ...
    return new Intl.NumberFormat('en-US', { // Use en-US for standard $ formatting
        style: 'currency',
        currency: 'USD',
...
export const getPayStatusColor = (status?: OrderPayStatus): ChipProps['color'] => {
    switch (status) {
        case 'Pendiente':
            return 'warning';
        case 'Abonado':
...
export const getCustomerName = (order?: OrderArchive | null): string => {
    ...
    if (order.basicData?.name) return order.basicData.name; // Company name?
    if (order.basicData?.firstname || order.basicData?.lastname) {
        return `${order.basicData.firstname || ''} ${order.basicData.lastname || ''}`.trim();
    }
