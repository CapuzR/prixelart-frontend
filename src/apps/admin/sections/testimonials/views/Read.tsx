...
const SortableTestimonialRow: React.FC<SortableTestimonialRowProps> = ({
    ...
    return (
        ...
        >
            {/* Drag Handle Cell */}
            <TableCell sx={{ width: '5%', cursor: 'grab', touchAction: 'none' }} {...attributes} {...listeners}>
                <Tooltip title="Arrastrar para reordenar">
                    <DragIndicatorIcon sx={{ verticalAlign: 'middle' }} />
                </Tooltip>
            </TableCell>
            {/* Original Cells */}
            <TableCell sx={{ fontWeight: 'medium', width: '5%' }}>{testimonial.position}</TableCell>
            <TableCell sx={{ width: '10%' }}>
                <Avatar src={testimonial.avatar} alt={testimonial.name} sx={{ width: 40, height: 40 }} imgProps={{ loading: 'lazy' }} />
            </TableCell>
            <TableCell component="th" scope="row" sx={{ width: '25%' }}>
                {testimonial.name}
                {testimonial.footer && <Typography variant="caption" display="block" color="text.secondary">{testimonial.footer}</Typography>}
            </TableCell>
            <TableCell sx={{ width: '15%' }}>{testimonial.type}</TableCell>
            <TableCell sx={{ width: '15%' }}>
                <Chip icon={testimonial.status ? <CheckCircleIcon /> : <CancelIcon />} label={testimonial.status ? 'Activo' : 'Inactivo'} color={testimonial.status ? 'success' : 'default'} size="small" variant="outlined" />
            </TableCell>
            <TableCell align="right" sx={{ width: '10%' }}>
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.5 }}>
                    <Tooltip title="Editar Testimonio">
                        <span>
                            <IconButton aria-label="edit" color="primary" onClick={() => handleUpdate(testimonial._id!.toString())} disabled={!testimonial._id || isDeleting} size="small">
                    ...
                    <Tooltip title="Eliminar Testimonio">
                        <span>
                            <IconButton aria-label="delete" color="error" onClick={() => handleOpenDeleteDialog(testimonial)} disabled={!testimonial._id || (isDeleting && testimonialToDeleteId === testimonial._id.toString())} size="small">
...
const ReadTestimonials: React.FC = () => {
    ...
    const sensors = useSensors(
        ...
        useSensor(KeyboardSensor, { // Use keyboard sensor for accessibility
            coordinateGetter: sortableKeyboardCoordinates,
        })
    );

    // --- Fetch Data ---
    const loadTestimonials = useCallback(async (showLoading = true) => {
        if (showLoading) setIsLoading(true);
        setError(null);
        try {
            const fetchedTestimonials = (await readAllTestimonial() as Testimonial[])
            ...
            for (const t of fetchedTestimonials) {
                if (t.position !== expectedPosition) {
                    console.warn(`Testimonial "${t.name}" has position ${t.position}, expected ${expectedPosition}. Data inconsistency detected.`);
                    needsReorder = true;
                }
                expectedPosition++;
            }
    ...
    const handleOpenDeleteDialog = (testimonial: Testimonial) => {
        if (!testimonial._id) { showSnackBar("No se puede eliminar: Falta ID."); return; }
        setTestimonialToDelete(testimonial); setDialogOpen(true);
    };
    const handleCloseDialog = () => { if (isDeleting || isSavingOrder) return; setDialogOpen(false); setTestimonialToDelete(null); };
    const handleConfirmDelete = async () => {
        if (!testimonialToDelete?._id) { showSnackBar("Error: Testimonio no seleccionado o falta ID."); setIsDeleting(false); handleCloseDialog(); return; }
        setIsDeleting(true);
        try {
    ...
    // --- Update Handling ---
    const handleUpdate = (testimonialId: string) => { if (!testimonialId) { showSnackBar("No se puede actualizar: Falta ID."); return; } navigate(`/admin/testimonials/update/${testimonialId}`); };
    // --- Create Handling ---
    const handleCreate = () => { navigate('/admin/testimonials/create'); };

    // --- Drag End Handling ---
    const handleDragEnd = async (event: DragEndEvent) => {
        const { active, over } = event;
        if (over && active.id !== over.id) {
            const oldIndex = testimonials.findIndex((t) => String(t._id) === active.id);
            ...
            const updatedTestimonialsWithPosition = reorderedTestimonials.map((t, index) => ({ ...t, position: index + 1 }));
            setTestimonials(updatedTestimonialsWithPosition); // Optimistic UI update

            setIsSavingOrder(true);
            setError(null);
            try {
                const orderedIds = updatedTestimonialsWithPosition.filter(t => t._id).map(t => t._id!.toString());
    ...
    const renderContent = () => {
        ...
        if (error && testimonials.length === 0) {
            return (
                <Alert severity="error" sx={{ m: 2 }}>
                    {error}
                    <Button onClick={() => loadTestimonials()} size="small" sx={{ ml: 1 }}>Reintentar</Button>
                </Alert>
            );
        }
        ...
        // Table content
        return (
            <TableContainer component={Paper} sx={{ mt: 2, opacity: isSavingOrder ? 0.7 : 1 }}>
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                    <SortableContext items={testimonials.filter(t => t._id).map(t => t._id!.toString())} strategy={verticalListSortingStrategy}>
                        <Table sx={{ minWidth: 650 }} aria-label="testimonials table">
                            <TableHead sx={{ backgroundColor: (theme) => theme.palette.action.hover }}>
                                <TableRow>
                                    <TableCell sx={{ width: '5%' }}></TableCell> {/* Drag Handle */}
                                    <TableCell sx={{ fontWeight: 'bold', width: '5%' }}>Pos.</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '10%' }}>Avatar</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '25%' }}>Nombre</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', width: '15%' }}>Tipo</TableCell>
                            ...
                            <TableBody>
                                {testimonials.map((testimonial) => (
                                    <SortableTestimonialRow
                                        ...
                                        testimonialToDeleteId={testimonialToDelete?._id?.toString() ?? null}
                                    />
                                ))}
                            </TableBody>
                        </Table>
    ...
    return (
        <>
            ...
            <Stack direction="row" justifyContent="flex-end" alignItems="center" mb={2} mt={1}>
                ...
                <Button variant="contained" startIcon={<AddIcon />} onClick={handleCreate} disabled={isLoading || isDeleting || isSavingOrder}>
                    Crear Testimonio
                </Button>
            </Stack>
            ...
            <ConfirmationDialog
                ...
                message={<>¿Estás seguro de que deseas eliminar el testimonio de <strong>{testimonialToDelete?.name || ''}</strong>? Esto puede afectar el orden.</>}
