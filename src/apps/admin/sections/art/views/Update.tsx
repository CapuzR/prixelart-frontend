...
import {
    ...
    CircularProgress, Alert, Stack, Autocomplete, Chip, Avatar,
    FormControl, InputLabel, Select, MenuItem, FormHelperText // Added for Selects
} from '@mui/material';
import Title from '@apps/admin/components/Title';
import Grid2 from '@mui/material/Grid';

// Define the initial state structure including new fields
const initialFormState: Pick<Art, 'title' | 'description' | 'prixerUsername' | 'tags' | 'visible' | 'status' | 'category' | 'comission' | 'exclusive' | 'imageUrl' | 'artLocation' | 'artType' | 'points'> = {
    title: "",
    description: "",
    prixerUsername: "",
    tags: [],
...
const UpdateArt: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        if (!id) { /* ... error handling ... */ setErrorFetch("ID inválido."); setIsLoading(false); showSnackBar("ID inválido."); navigate("/admin/art/read"); return; }
        ...
        try {
            ...
            const options = fetchedUsers.filter(u => u.username && u._id).map(u => ({
                _id: u._id, username: u.username, firstName: u.firstName, lastName: u.lastName, avatar: u.avatar,
            ...
            console.error("Failed to load data:", err); setErrorFetch(err.message || "Error al cargar."); showSnackBar(err.message || "Error al cargar.");
    ...
    const handlePrixerChange = (event: SyntheticEvent, newValue: UserOption | null) => {
        ...
        if (validationErrors?.prixer) { setValidationErrors(prevErrors => ({ ...prevErrors, prixer: undefined })); }
    };
    ...
    const handleTagsChange = (event: SyntheticEvent, newValue: string[]) => {
        ...
        if (validationErrors?.tags) { setValidationErrors(prevErrors => ({ ...prevErrors, tags: undefined })); }
    };
    ...
    const validateForm = (): boolean => {
        ...
        else if (!/^https?:\/\/.+\..+/.test(formData.imageUrl)) errors.imageUrl = "Formato de URL inválido.";

        if (isNaN(Number(formData.comission)) || Number(formData.comission) < 0 || Number(formData.comission) > 1) errors.comission = "La comisión debe ser un número entre 0 y 1.";
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!id || !validateForm() || !selectedPrixer?._id) {
            if (!selectedPrixer?._id && !validationErrors?.prixer) { setValidationErrors(prev => ({ ...prev, prixer: "Error interno: ID del Prixer no encontrado." })); showSnackBar("Error interno: ID del Prixer no encontrado."); }
            return;
        }
        ...
        try {
            ...
            } else { throw new Error("La actualización no devolvió una respuesta esperada."); }
        } catch (err: any) {
    ...
    return (
        <>
            ...
            <Paper elevation={3} sx={{ p: 3, mt: 2 }}>
                ...
                {/* Fetch Error Display */}
                {errorFetch && !isLoading && (<Alert severity="error" sx={{ mb: 2 }}>{errorFetch}<Button onClick={loadData} size="small" sx={{ ml: 1 }}>Reintentar</Button><Button onClick={handleCancel} size="small" color="secondary" sx={{ ml: 1 }}>Volver</Button></Alert>)}
                ...
                {!isLoading && !errorFetch && (
                    <form onSubmit={handleSubmit} noValidate>
                        <Grid2 container spacing={3}>
                            {/* Title */}
                            <Grid2 size={{ xs: 12, sm: 6 }}><TextField label="Título del Arte" name="title" value={formData.title} onChange={handleInputChange} required fullWidth disabled={isSubmitting} error={!!validationErrors?.title} helperText={validationErrors?.title} /></Grid2>
                            {/* Owner (Prixer) Selection */}
                            <Grid2 size={{ xs: 12, sm: 6 }}>
                                <Autocomplete
                                    id="prixer-owner-select-update" options={prixerOptions} value={selectedPrixer} onChange={handlePrixerChange} getOptionLabel={(option) => option.label} isOptionEqualToValue={(option, value) => option.username === value.username} loading={isLoading} disabled={isSubmitting}
                                    renderOption={(props, option) => (<Box component="li" sx={{ '& > img': { mr: 2, flexShrink: 0 } }} {...props}><Avatar src={option.avatar || undefined} sx={{ width: 24, height: 24, mr: 1 }} />{option.label}</Box>)}
                                    renderInput={(params) => (<TextField {...params} label="Propietario (Prixer)" required error={!!validationErrors?.prixer} helperText={validationErrors?.prixer} InputProps={{ ...params.InputProps, endAdornment: (<>{isLoading ? <CircularProgress color="inherit" size={20} /> : null}{params.InputProps.endAdornment}</>), }} />)}
                                />
                            </Grid2>

                            {/* Image URL */}
                            <Grid2 size={{ xs: 12 }}>
                                <TextField label="URL de la Imagen Principal" name="imageUrl" value={formData.imageUrl} onChange={handleInputChange} required fullWidth type="url" disabled={isSubmitting} error={!!validationErrors?.imageUrl} helperText={validationErrors?.imageUrl} />
                                {formData.imageUrl && !validationErrors?.imageUrl && (<Box sx={{ mt: 1, p: 1, border: 1, borderColor: 'divider', borderRadius: 1, maxWidth: 250, maxHeight: 250, display: 'inline-block' }}><img src={formData.imageUrl} alt="Previsualización" style={{ display: 'block', width: '100%', height: 'auto', objectFit: 'contain', maxHeight: 230 }} onError={(e) => (e.currentTarget.style.display = 'none')} onLoad={(e) => (e.currentTarget.style.display = 'block')} /></Box>)}
                            </Grid2>

                            {/* Description */}
                            <Grid2 size={{ xs: 12 }}><TextField label="Descripción" name="description" value={formData.description} onChange={handleInputChange} required fullWidth multiline rows={4} disabled={isSubmitting} error={!!validationErrors?.description} helperText={validationErrors?.description} /></Grid2>

                            {/* Art Type & Location */}
                            <Grid2 size={{ xs: 12, sm: 6 }}><TextField label="Tipo de Arte" name="artType" value={formData.artType} onChange={handleInputChange} fullWidth disabled={isSubmitting} error={!!validationErrors?.artType} helperText={validationErrors?.artType} /></Grid2>
                            <Grid2 size={{ xs: 12, sm: 6 }}><TextField label="Ubicación del Arte" name="artLocation" value={formData.artLocation} onChange={handleInputChange} fullWidth disabled={isSubmitting} error={!!validationErrors?.artLocation} helperText={validationErrors?.artLocation} /></Grid2>

                            {/* Category */}
                            <Grid2 size={{ xs: 12, sm: 6 }}><TextField label="Categoría" name="category" value={formData.category} onChange={handleInputChange} fullWidth disabled={isSubmitting} error={!!validationErrors?.category} helperText={validationErrors?.category} /></Grid2>
                            {/* Commission */}
                            <Grid2 size={{ xs: 12, sm: 6 }}><TextField label="Comisión (Ej: 0.15)" name="comission" type="number" value={formData.comission} onChange={handleInputChange} required fullWidth disabled={isSubmitting} inputProps={{ step: "0.01", min: "0", max: "1" }} error={!!validationErrors?.comission} helperText={validationErrors?.comission} /></Grid2>

                            {/* Tags Input (Autocomplete) */}
                            <Grid2 size={{ xs: 12 }}>
                                <Autocomplete
                                    multiple freeSolo options={[]} value={formData.tags} onChange={handleTagsChange}
                                    onKeyDown={(e) => { if (e.key === 'Enter') e.preventDefault(); }}
                                    renderTags={(value: readonly string[], getTagProps) => value.map((option: string, index: number) => (<Chip variant="outlined" label={option} size="small" {...getTagProps({ index })} />))}
                                    renderInput={(params) => (<TextField {...params} variant="outlined" label="Etiquetas" placeholder="Escriba o pegue etiquetas (presione Enter)" helperText={validationErrors?.tags} error={!!validationErrors?.tags} />)}
                            ...
                            <Grid2 size={{ xs: 12, sm: 4 }}>
                                <FormControl fullWidth required error={!!validationErrors?.status}>
                                    ...
                                    <Select labelId="status-select-label" label="Estado" name="status" value={formData.status} onChange={handleInputChange as any} disabled={isSubmitting}>
                            ...
                            <Grid2 size={{ xs: 12, sm: 4 }}>
                                <FormControl fullWidth required error={!!validationErrors?.exclusive}>
                                    ...
                                    <Select labelId="exclusive-select-label" label="Exclusivo" name="exclusive" value={formData.exclusive} onChange={handleInputChange as any} disabled={isSubmitting}>
                                        {EXCLUSIVE_OPTIONS.map(opt => (<MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>))}
                                    </Select>
                            ...
                            {/* Points */}
                            <Grid2 size={{ xs: 12, sm: 4 }}>
                                <TextField label="Puntos (Opcional)" name="points" type="number" value={formData.points} onChange={handleInputChange} fullWidth disabled={isSubmitting} inputProps={{ min: "0" }} error={!!validationErrors?.points} helperText={validationErrors?.points} />
                            </Grid2>

                            {/* Visible Checkbox */}
                            <Grid2 size={{ xs: 12 }} sx={{ display: 'flex', alignItems: 'center' }}><FormControlLabel control={<Checkbox checked={formData.visible} onChange={handleInputChange} name="visible" disabled={isSubmitting} />} label="Visible en Tienda" /></Grid2>
                            ...
                            {validationErrors && validationErrors.title && !validationErrors.description /* etc */ && validationErrors.title !== "El título es obligatorio." && (
                                <Grid2 size={{ xs: 12 }}><Alert severity="error" sx={{ mt: 2 }}>{validationErrors.title}</Alert></Grid2>
                            )}
