...
import { useLoading, useUser } from "@prixpon/context/GlobalContext";
...
import {
  ...
  Variant,
  VariantAttribute,
} from "@prixpon/types/product.types"; // Import VariantAttribute
import {
...
const Details: React.FC<DetailsProps> = ({ productId }) => {
  ...
  useEffect(() => {
    if (!product?._id || !product.variants || product.variants.length === 0) {
      setCalculatedRangeInfo({
        ...
        finalMax: null,
        isLoading: false,
        error: product ? "Producto sin variantes." : null,
      });
      return;
    }
    ...
    const calculatePrices = async () => {
      ...
      try {
        const allVariantPrices = await fetchAllVariantPricesForProduct(
        ...
        if (allVariantPrices.length === 0) {
          throw new Error(
            "No se pudieron determinar los precios para ninguna variante.",
          );
        }

        const isPrixer = user?.prixer;
        const basePriceField: keyof Variant =
          isPrixer && "prixerPrice" in product.variants![0]
            ? "prixerPrice"
            : "publicPrice";

        const basePrices = product
          .variants!.map((v) => {
            const priceValue = v[basePriceField];
            return priceValue != null
              ? formatNumberString(String(priceValue))
              : NaN;
          })
          .filter((p) => !isNaN(p));
        ...
        console.error(
          "Error al calcular el rango de precios del producto:",
          err,
        );
        if (isMounted) {
          setCalculatedRangeInfo({
            ...
            error:
              err instanceof Error
                ? err.message
                : "Error al calcular rango de precios.",
          });
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const foundVariant = getSelectedVariant(
      variantAttributes,
      product.variants,
    );
        ...
        try {
          ...
          if (isMounted) {
            if (priceResult && priceResult.length === 2) {
              ...
              throw new Error(
                "No se pudo obtener el precio para la variante seleccionada.",
              );
            }
          }
        } catch (error) {
  ...
  function handleArtSelection(): void {
    ...
    const variantAttributesObject: { [key: string]: string } =
  ...
  return (
    <div
      style={{
        maxHeight: `${windowHeight - (productId ? 0 : 64)}px`,
        overflowY: "auto",
      }}
    >
      {product ? (
      ...
      calculatedRangeInfo.isLoading ||
        calculatedRangeInfo.error ? null : null // Or a dedicated loading/error view
      }
    </div>
  );
};
