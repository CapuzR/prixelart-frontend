...
export const getUserById = async (id: string): Promise<User> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure fetching user '${id}':`, response.data.message);
...
export const getUsersByIds = async (ids: string[]): Promise<User[]> => {
    ...
    try {
        const response = await axios.post<PrixResponse>(base_url, { ids: ids }, { withCredentials: true },);
...
export const updateUser = async (id: string, userData: Partial<User>): Promise<User> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure updating user '${userData.username}':`, response.data.message);
            throw new Error(response.data.message || `Failed to update user '${userData.username}'`);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no user data returned after update for '${userData.username}'.`);
            throw new Error(`No data received after updating user '${userData.username}'.`);
        }
...
export const deleteUser = async (username: string): Promise<User> => {
    const base_url =
        import.meta.env.VITE_BACKEND_URL + "/user/delete/" + username
    try {
