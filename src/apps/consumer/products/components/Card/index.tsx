...
export default function Card({ product, currency, conversionRate, handleDetails, isCart, onProductSelect }: CardProps) {
  ...
  useEffect(() => {
    if (!product?._id || !product.variants || product.variants.length === 0) {
      setPriceInfo({
        ...
        isLoading: false, error: 'Producto sin variantes válidas.',
      });
      return;
    }
    ...
    const calculatePrices = async () => {
      ...
      try {
        ...
        const pricePromises = (product.variants ?? []).map(async (variant): Promise<[number, number] | null> => {
          // Check if discountId array exists and is not empty
          const hasDiscount = Array.isArray(variant.discountId) && variant.discountId.length > 0;

          if (hasDiscount && variant._id && product._id) {
            // Fetch potentially discounted price
            const fetchedPrices = await fetchVariantPrice(variant._id, product._id.toString());
        ...
        const validFinalPrices = results
          .filter((result): result is [number, number] => result !== null && !isNaN(result[1])) // Ensure result exists and final price is a number
        ...
        if (isMounted) {
          setPriceInfo({
            baseMin: null, baseMax: null, finalMin: null, finalMax: null,
            isLoading: false,
            error: err instanceof Error ? err.message : 'Error al calcular precios.',
          });
        }
      }
    };
  ...
  const renderPrice = () => {
    ...
    const finalPriceString = formatRange(priceInfo.finalMin, priceInfo.finalMax, currency, conversionRate);
    ...
    if (baseRangeDiffers) {
      ...
      const basePriceString = formatRange(priceInfo.baseMin, priceInfo.baseMax, currency, conversionRate);
      ...
      return <span dangerouslySetInnerHTML={{ __html: `<span>${finalPriceString}</span>` }} />;
    }
  };
