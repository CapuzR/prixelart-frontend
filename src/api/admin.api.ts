...
export const getPermissions = async (): Promise<Permissions> => {
    try {
        ...
        if (response.data.result === undefined || response.data.result === null) {
            console.error('Backend reported success but no permissions result was provided.');
...
export const getAdminByUsername = async (username: string): Promise<Admin> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure fetching admin '${username}':`, response.data.message);
            throw new Error(response.data.message || `Failed to fetch admin '${username}'`);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no admin data for '${username}'.`);
...
export const updateAdmin = async (id: string, adminData: Partial<Admin>): Promise<Admin> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure updating admin '${adminData.username}':`, response.data.message);
            throw new Error(response.data.message || `Failed to update admin '${adminData.username}'`);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no admin data returned after update for '${adminData.username}'.`);
            throw new Error(`No data received after updating admin '${adminData.username}'.`);
        }
...
export const getRoleById = async (id: string): Promise<Permissions> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure fetching role '${id}':`, response.data.message);
...
export const updateRole = async (id: string, roleData: Partial<Permissions>): Promise<Permissions> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure updating role '${id}':`, response.data.message);
            throw new Error(response.data.message || `Failed to update role '${id}'`);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no role data returned after update for '${id}'.`);
...
export const deleteRole = async (id: string): Promise<Permissions> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure deleting role '${id}':`, response.data.message);
