...
export const createCarouselItem = async (itemData: { type: 'desktop' | 'mobile'; imageURL: string; }): Promise<CarouselItem> => {
    const URI = `${BACKEND_URL}/carousel`;

    try {
        const response = await axios.post<CreateCarouselItemPrixResponse>(URI, itemData, {
        ...
        if (response.data && response.data.success && response.data.result) {
            ...
            const errorMessage = response.data?.message || 'Failed to create carousel item due to an unknown server error.';
            console.error('Error in createCarouselItem API call (server-side):', errorMessage, response.data);
...
export const deleteCarouselItem = async (id: string): Promise<PrixResponse> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure deleting carousel item ${id}:`, response.data.message);
            throw new Error(response.data.message || `Failed to delete carousel item.`);
        }
...
export const updateCarouselOrder = async (type: 'desktop' | 'mobile', orderedIds: string[]): Promise<PrixResponse> => {
    ...
    try {
        ...
        const response = await axios.put<PrixResponse>(URI, { orderedIds }, { // Send orderedIds in the request body
            withCredentials: true, // Include credentials
        });

        // Check for backend success indication
        if (!response.data.success) {
            console.error(`Backend reported failure updating ${type} carousel order:`, response.data.message);
            throw new Error(response.data.message || `Failed to update ${type} carousel order.`);
        }
...
export const updateTerms = async (terms: TermsAndConditions): Promise<TermsAndConditions> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure updating terms`, response.data.message);
            throw new Error(response.data.message || `Failed to update terms`);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no terms data returned after update.`);
            throw new Error(`No data received after updating terms.`);
        }

        const updatedTermsAndConditions = response.data.result as unknown as TermsAndConditions;
