...
export const fetchSurchargeById = async (
  surchargeId: string,
): Promise<Surcharge> => {
...
export const fetchActiveSurcharges = async (): Promise<Surcharge[]> => {
  const base_url =
    import.meta.env.VITE_BACKEND_URL + "/surcharge/read-all-active";
  try {
    const response = await axios.get<PrixResponse>(base_url, {
      withCredentials: true,
    });
...
export const updateSurcharge = async (
  id: string,
  surchargeData: Partial<Surcharge>,
): Promise<Surcharge> => {
  ...
  try {
    ...
    if (!response.data.success) {
      console.error(
        `Backend reported failure updating surcharge`,
        response.data.message,
      );
    ...
    if (!response.data.result) {
      console.error(
        `Backend reported success but no surcharge data returned after update.`,
      );
...
export const deleteSurcharge = async (id: string): Promise<Surcharge> => {
  const base_url = import.meta.env.VITE_BACKEND_URL + "/surcharge/delete/" + id;
  try {
