...
const Portrait: React.FC<PortraitProps> = (props) => {
  ...
  const [step, setStep] = useState<number>(
    props.searchParams.get('step') ? parseInt(props.searchParams.get('step') as string) : 1
  );
  ...
  const handleSelect = (e: React.ChangeEvent<{ name: string; value: number }>) => {
    ...
    // Object.keys(props.product?.selection).every((key) => props.product?.selection[key] !== '') &&
    //   setStep(2);
            ...
            <div className={styles['select']}>
              ...
              <div
                className={`${styles['attributes-container']} ${
                  props.product?.attributes?.length > 1
            ...
            <div className={styles['info-accordion-wrapper']}>
              ...
              >
                <AccordionSummary
                  ...
                  id="panel2a-header"
                >
                  <Typography className={styles['accordion-title']}>
                    {' '}
                ...
                <AccordionDetails className={styles['accordion-details']}>
                  <MDEditor.Markdown
                    source={
                      props.technicalSpecification || 'No technical specifications available.'
                    }
                    className={styles['markdown-content']}
                  />
                </AccordionDetails>
