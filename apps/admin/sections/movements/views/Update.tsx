...
const UpdateMovement: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    if (!id) {
      setErrorFetch("No se proporcionó ID del movimiento.");
      setIsLoading(false);
    ...
    try {
      ...
      // Populate form data
      const { _id, date, createdOn, createdBy, item, ...movementFormData } =
        movementData; // Exclude non-form fields
      setFormData({
      ...
      const currentDestinatary = options.find(
        (opt) => opt.account === movementData.destinatary,
      );
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload: Partial<
      Pick<Movement, "description" | "order" /* | 'item' */>
    > = {
    ...
    try {
      ...
      if (response) {
        ...
        throw new Error(
          "La actualización del movimiento no devolvió una respuesta esperada.",
        );
      }
    } catch (err: any) {
  ...
  return (
    <>
      <Title
        title={`Actualizar Movimiento: ${originalMovementDesc.substring(0, 30)}...`}
      />
