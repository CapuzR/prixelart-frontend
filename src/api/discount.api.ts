...
export const createDiscount = async (data: Partial<Discount>): Promise<Discount> => {
...
export const fetchActiveDiscounts = async (): Promise<Discount[]> => {
    const base_url = import.meta.env.VITE_BACKEND_URL + "/discount/read-all-active";
    try {
        const response = await axios.get<PrixResponse>(base_url, { withCredentials: true },);
...
export const updateDiscount = async (id: string, discountData: Partial<Discount>): Promise<Discount> => {
    ...
    try {
        ...
        if (!response.data.success) {
            console.error(`Backend reported failure updating discount`, response.data.message);
            throw new Error(response.data.message || `Failed to update discount`);
        }

        if (!response.data.result) {
            console.error(`Backend reported success but no discount data returned after update.`);
...
export const deleteDiscount = async (id: string): Promise<Discount> => {
    const base_url =
        import.meta.env.VITE_BACKEND_URL + "/discount/delete/" + id
    try {
