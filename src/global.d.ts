...
declare module '@uppy/aws-s3-multipart' {
  export interface AwsS3MultipartOptions {
    ...
    createMultipartUpload?: (file: UppyFile) => Promise<CreateMultipartUploadResponse>;
    signPart?: (file: UppyFile, params: { uploadId: string, key: string, partNumber: number, body: Blob, signal: AbortSignal }) => Promise<SignPartResponse>;
    completeMultipartUpload?: (file: UppyFile, params: { uploadId: string, key: string, parts: AwsS3Part[], signal: AbortSignal }) => Promise<CompleteMultipartUploadResponse>;
    abortMultipartUpload?: (file: UppyFile, params: { uploadId: string, key: string, signal: AbortSignal }) => Promise<void>;
    listParts?: (file: UppyFile, params: { uploadId: string, key: string, signal: AbortSignal }) => Promise<AwsS3Part[]>; // Optional but good for resumability
  }
