...
import { Product, Variant } from 'types/product.types'; // Added Variant
import { fetchProducts } from '@api/product.api';
import { fetchDiscountById, updateDiscount } from '@api/discount.api';
...
import {
    ...
    FormHelperText, Autocomplete, Chip, Divider,
    IconButton, Tooltip,
...
const UpdateDiscount: React.FC = () => {
    ...
    const [expandedAccordion, setExpandedAccordion] = useState<string | false>('applicability'); // Default open
    ...
    const productVariantOptions = useMemo((): ProductOrVariantOption[] => {
        ...
        allProducts.forEach(product => {
            ...
            product.variants.forEach(variant => { /* ... price calculation logic ... */
            ...
            if (validPricesFound) { /* ... formatRange logic ... */
            ...
            product.variants.forEach(variant => { /* ... add variant option with price in label ... */
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            if (!id) {
                setErrorFetch("ID de descuento inválido."); setIsLoadingData(false); setIsLoadingProducts(false); showSnackBar("ID de descuento inválido."); navigate("/admin/discount/read"); return;
            }
            ...
            try {
                ...
                setFormData({
                    ...
                    appliesToAllProducts: discountData.appliesToAllProducts ?? (discountData.applicableProducts || []).length === 0,
                ...
                setStartDate(discountData.dateRange?.start ? dayjs(discountData.dateRange.start) : null);
                setEndDate(discountData.dateRange?.end ? dayjs(discountData.dateRange.end) : null);
    ...
    useEffect(() => {
        ...
        if (!isLoadingData && !isLoadingProducts && originalDiscount && allProducts.length > 0 && productVariantOptions.length > 0) {

            const initialApplicableVariants = new Set<string>(); // Use set for "productId_variantId"

            if (!originalDiscount.appliesToAllProducts && originalDiscount.applicableProducts) {
            ...
            const finalInitialApplicableProducts: [string, string][] = Array.from(initialApplicableVariants).map(idPair => {
            ...
            const preselectedOptions = productVariantOptions.filter(opt =>
                !opt.isProduct && initialApplicableVariants.has(opt.id) // Match based on the composite "productId_variantId"
            );
            ...
            if (finalInitialApplicableProducts.length > 0 || (originalDiscount?.entityOverrides?.length ?? 0 > 0) || originalDiscount?.dateRange) {
    ...
    }, [isLoadingData, isLoadingProducts, originalDiscount, allProducts, productVariantOptions]); // Dependencies

    // --- Handlers (Copied/Adapted from refactored CreateDiscount) ---
    const handleInputChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        const target = event.target as HTMLInputElement; const { name, value, type } = target; const checked = target.checked; const fieldVal = type === "checkbox" ? checked : value;
        ...
        if (validationErrors && validationErrors[name as keyof DiscountValidationErrors]) {
            setValidationErrors(prevErrors => {
                ...
                if (name === 'defaultValue' || name === 'adjustmentMethod') { delete updated.priceValidation; }
                return Object.keys(updated).length > 0 ? updated : null;
            });
        }
    };
    const handleSelectChange = (event: ChangeEvent<{ name?: string; value: unknown }>) => {
        const name = event.target.name as keyof typeof formData; const value = event.target.value;
    ...
    const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        // Clear validation
        if (name === 'appliesToAllProducts' && (validationErrors?.applicableProducts || validationErrors?.priceValidation)) {
            setValidationErrors(prevErrors => {
                if (!prevErrors) return null;
                const updated = { ...prevErrors };
    ...
    const handleProductVariantChange = (event: SyntheticEvent, newValue: ProductOrVariantOption[]) => {
        ...
        if (validationErrors?.applicableProducts || validationErrors?.priceValidation) {
            setValidationErrors(prevErrors => {
                if (!prevErrors) return null;
                const updated = { ...prevErrors };
                delete updated.applicableProducts; delete updated.priceValidation;
                return Object.keys(updated).length > 0 ? updated : null;
            });
        }
    };

    const handleStartDateChange = (newValue: Dayjs | null) => { /* ... clear date validation ... */ };
    const handleEndDateChange = (newValue: Dayjs | null) => { /* ... clear date validation ... */ };
    const handleAddOverride = () => { /* ... add override, clear override validation ... */ };
    const handleOverrideChange = (tempId: string, field: keyof ApplicableEntity, value: string | number | Entity | AdjustmentMethod) => { /* ... update override, clear specific + price validation ... */ };
    const handleRemoveOverride = (tempIdToRemove: string) => { /* ... remove override, clear price validation ... */ };
    const handleAccordionChange = (panel: string) => (event: React.SyntheticEvent, isExpanded: boolean) => { /* ... set accordion state ... */ };
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.appliesToAllProducts && formData.applicableProducts.length > 0 && !errors.defaultValue) {
            ...
            allProducts.forEach(p => { if (p._id && p.variants) p.variants.forEach(v => { if (v._id) variantMap.set(`${p._id}_${v._id}`, v); }); });
            ...
            for (const [productId, variantId] of formData.applicableProducts) {
                ...
                if ((discountedPublic !== null && discountedPublic <= 0) || (discountedPrixer !== null && discountedPrixer <= 0)) {
                    const productName = allProducts.find(p => p._id!.toString() === productId)?.name || 'Producto';
        ...
        if (Object.keys(errors).length > 0 && !errors.summary) { errors.summary = `Por favor, corrija los errores marcados.`; setValidationErrors(errors) }
        if (Object.keys(errors).length > 0 && errors.summary) { if (!errors.priceValidation || Object.keys(errors).length > 1) showSnackBar("Por favor, corrija los errores indicados."); }
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        const payload: Omit<Discount, '_id'> = {
            ...
            applicableProducts: formData.appliesToAllProducts ? [] : formData.applicableProducts, // Use current formData
        ...
        try {
            ...
            if (response) { showSnackBar(`Descuento "${formData.name}" actualizado.`); navigate("/admin/discount/read"); }
            else { throw new Error("La actualización del descuento falló (respuesta vacía)."); }
        } catch (err: any) {
            console.error("Failed to update discount:", err);
            const message = err?.response?.data?.message || err.message || "Error al actualizar el descuento.";
    ...
    return (
        <>
            <Title title={`Actualizar Descuento: ${originalDiscount?.name || (id ? 'Cargando...' : 'Inválido')}`} />
            <Paper elevation={3} sx={{ p: { xs: 2, sm: 3 }, mt: 2 }}>
                {isLoading && !errorFetch && (<Box sx={{ display: 'flex', justifyContent: 'center', p: 3 }}><CircularProgress /></Box>)}
                {errorFetch && (
                ...
                {!isLoading && !errorFetch && originalDiscount && ( // Render form only when data is ready
                    <form onSubmit={handleSubmit} noValidate>
                        ...
                        <Grid2 container spacing={3}>
                            ...
                            <Grid2 size={{ xs: 12, sm: 6 }}>{/* Adjustment Method Select */}</Grid2>
                            <Grid2 size={{ xs: 12, sm: 6 }}>{/* Default Value TextField */}</Grid2>
                                                        ...
                                                        <Autocomplete
                                                            ...
                                                            renderInput={(params) => (
                                                                <TextField
                                                                    ...
                                                                    placeholder={isLoadingProducts ? "Cargando Productos..." : "Seleccionar..."}
                                            ...
                                            return (
                                                ...
                                                <Box key={override.tempId} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, p: 2, mb: 2, position: 'relative' }}>
                                                    <Tooltip title="Eliminar Excepción">
                                                        <IconButton size="small" onClick={() => handleRemoveOverride(override.tempId)} disabled={isSubmitting} sx={{ position: 'absolute', top: 8, right: 8 }} aria-label="Eliminar Excepción">
                                                    ...
                                                    <Grid2 container spacing={2}>
                                                        ...
                                                        <Grid2 size={{ xs: 12, sm: 6, md: 3 }}>
                                                            <TextField
                                                                ...
                                                                onChange={(e) => handleOverrideChange(override.tempId, 'customValue', e.target.value)} // Pass string value
