...
export default function UpdateOrder() {
  ...
  const [imageToCropDetails, setImageToCropDetails] = useState<{
    originalFile: File
    targetType: "paymentVoucher" // Tipo específico para comprobantes
    tempId: string
  } | null>(null)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const transformedLines: OrderLineFormState[] = orderData.lines.map(
        (line) => {
          /* ... (sin cambios en la transformación de líneas) ... */ const selectedProductOpt =
            productOpts.find(
          ...
          return {
            ...
            selectedArt:
              artOpts.find((a) => a.id === line.item?.art?._id?.toString()) ||
              null,
  ...
  const closeAndResetCropper = () => {
    ...
    // Resetear el input file si se cancela el recorte
      ...
      <form onSubmit={handleSubmit} id="update-order-form">
                          ...
                          <Grid2 container spacing={1.5} alignItems="center">
                            <Grid2 size={{ xs: 12, md: 6 }}>
                              <Autocomplete
                                ...
                                onChange={(e, v) =>
                                  handleArtSelection(line.tempId, v)
                                }
        ...
        {activeStep === 1 && (
          <Box sx={{ mt: 3 }}>
            ...
            {/* Typography original que pediste mantener como referencia */}
            {/* <Typography>XO</Typography>  */}
          </Box>
        )}
        <Stack
          direction="row"
        ...
        >
          ...
          {activeStep === steps.length - 1 ? (
            ...
            >
              {isSubmitting ? "Guardando..." : "Guardar Cambios en Orden"}
            </Button>
          ) : (
            <Button
              onClick={handleNext}
