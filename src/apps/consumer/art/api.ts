...
export const fetchArtsByPrixer = async (username: string): Promise<Art[]> => {
  const res = await axios.post<PrixResponse>(`${BACKEND_URL}/art/read-by-prixer`, { username });
  return res.data.result as Art[];
};

export const fetchTermsContent = async (): Promise<string> => {
  const res = await axios.get<{ terms: { termsAndConditions: string } }>(`${BACKEND_URL}/termsAndConditions/read`);
  return res.data.terms.termsAndConditions;
};

export const fetchPriserTermsAgree = async (username: string): Promise<boolean> => {
  const res = await axios.get<{ termsAgree: boolean }>(`${BACKEND_URL}/prixer/get/${username}`);
...
export const deleteArt = async (artId: string): Promise<PrixResponse> => {
  const res = await axios.delete<PrixResponse>(`${BACKEND_URL}/art/delete/${artId}`);
  return res.data;
};

export const toggleArtVisibility = async (artId: string, disabledReason: string, adminToken: string): Promise<PrixResponse> => {
  const res = await axios.put<PrixResponse>(
...
export const rankArt = async (art: Art, points: number, certificate: { code: string; serial: number; sequence: number }, adminToken: string): Promise<PrixResponse> => {
