...
function getNestedValue(obj: any, path: string): any {
  ...
  return value === null || typeof value === "undefined"
    ? typeof value === "number"
      ? -Infinity
      : ""
    : value; // Handle nulls consistently
}
...
const formatApplicability = (discount: Discount): string => {
  ...
  if (discount.appliesToAllProducts) parts.push("Todos los Productos");
    ...
    parts.push(`${discount.applicableProducts.length} Prod.`);

  if (discount.appliestoAllArts) parts.push("Todas las Artes");
  else if (discount.applicableArts && discount.applicableArts.length > 0)
    parts.push(`${discount.applicableArts.length} Arte(s)`);

  if (
    parts.length === 0 &&
    !(discount.entityOverrides && discount.entityOverrides.length > 0)
  )
    return "N/A"; // Maybe only overrides apply
  if (
    parts.length === 0 &&
    discount.entityOverrides &&
    discount.entityOverrides.length > 0
  )
    return "Solo Entidades";
  if (
    parts.includes("Todos los Productos") &&
    parts.includes("Todas las Artes")
  )
    return "Todo";
...
const ReadDiscounts: React.FC = () => {
  ...
  const loadDiscounts = useCallback(
    async (showLoading = true) => {
      ...
      try {
        ...
        /* ... error handling ... */
        const message = err.message || "Error al cargar descuentos.";
        setError(message);
  ...
  const filteredDiscounts = useMemo(() => {
    ...
    return discounts.filter((discount) => {
      ...
      const statusMatch =
        filterStatus === "all" ||
        (discount.active ? "active" : "inactive") === filterStatus;

      // Method Filter
      const methodMatch =
        filterMethod === "all" || discount.adjustmentMethod === filterMethod;

      // Currently Active Filter
      let currentlyActiveMatch = true;
      if (filterCurrentlyActive !== "all") {
        const hasDateRange =
          !!discount.dateRange?.start && !!discount.dateRange?.end;
        if (!hasDateRange) {
  ...
  const handleFilterChange = (
    ...
    // Use 'any' for simplicity with different filter types
  ...
  const handleUpdate = (discountId: string | undefined) => {
    if (discountId) navigate(`/admin/discount/update/${discountId}`);
  };
  ...
  return (
    <>
      ...
      {error && !isLoading && filteredDiscounts.length === 0 && (
        <Alert severity="error" sx={{ m: 2 }}>
          {error}
          <Button onClick={() => loadDiscounts()} size="small" sx={{ ml: 1 }}>
            Reintentar
          </Button>
        </Alert>
      )}
      ...
      {!isLoading && filteredDiscounts.length === 0 && !error && (
        <Alert severity="info" sx={{ m: 2 }}>
          No se encontraron descuentos que coincidan con los criterios.
        </Alert>
      )}
            ...
            >
              <TableRow>
                ...
                <TableCell sx={{ width: "15%", fontWeight: "bold" }}>
                  <TableSortLabel
                    active={orderBy === "dateRange.start"}
              ...
              {paginatedDiscounts.map((discount) => (
                ...
                >
                  <TableCell component="th" scope="row">
                    ...
                    >
                      ...
                      {discount.entityOverrides &&
                        discount.entityOverrides.length > 0 && (
                          <Tooltip
                            title={`Tiene ${discount.entityOverrides.length} excepción(es)`}
                          >
                            <InfoOutlinedIcon
                              color="action"
                  ...
                  <TableCell>
                    <Chip
                      icon={
                        discount.active ? <CheckCircleIcon /> : <CancelIcon />
                      }
                      label={discount.active ? "Activo" : "Inactivo"}
                    ...
                    >
                      <Tooltip title="Editar Descuento">
                        <IconButton
                          aria-label="edit"
                          color="primary"
                          onClick={() => handleUpdate(discount._id?.toString())}
                          disabled={!discount._id || isDeleting}
                          size="small"
                        >
                      ...
                      <Tooltip title="Eliminar Descuento">
                        <IconButton
                          ...
                          disabled={
                            !discount._id ||
                            (isDeleting &&
                              discountToDelete?._id === discount._id)
                          }
                          size="small"
                        >
